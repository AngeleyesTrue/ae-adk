...
install_binary() {
    ...
    if [ "$OS" = "darwin" ]; then
        ...
        print_info "Removed macOS quarantine flag"
    fi
}
...
verify_installation() {
    if command -v ae &> /dev/null; then
        ...
        echo "  ae init          # Initialize a new project"
        echo "  ae doctor        # Check system health"
        echo "  ae update --project # Update project templates"
    else
        print_warning "Installation completed, but 'ae' command not found in PATH"
