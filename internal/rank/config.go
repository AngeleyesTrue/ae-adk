...
func NewConfig() *Config {
	...
	if envURL := os.Getenv("AE_RANK_API_URL"); envURL != "" {
		cfg.BaseURL = envURL
	}
	return cfg
}
...
// Credentials are stored at ~/.ae/rank/credentials.json.
...
// If dir is empty, it defaults to ~/.ae/rank/.
func NewFileCredentialStore(dir string) *FileCredentialStore {
	if dir == "" {
		...
		dir = filepath.Join(home, defs.AEDir, defs.RankSubdir)
	}
