...
//
// Regression test for: https://github.com/AngeleyesTrue/ae-adk/issues/443
//
// The builder-skill agent was creating files at wrong nested paths like:
//
//	.claude/skills/ae/library/pykrx.md  (WRONG)
//
...
func TestBuilderSkillPathStructure(t *testing.T) {
	...
	const agentPath = ".claude/agents/ae/builder-skill.md"
	...
	// .claude/skills/. The full skill name (e.g. ae-library-pykrx) must
	...
	// hyphens (e.g., ae-library-pykrx → .claude/skills/ae-library-pykrx/SKILL.md).
	...
	hasConcreteExample := strings.Contains(content, ".claude/skills/{skill-name}/SKILL.md") ||
		...
		strings.Contains(content, ".claude/skills/ae-library-") ||
