...
// into the ae binary at compile time via //go:embed. This includes agent
...
// The all: prefix ensures dot-prefixed directories (.claude/, .ae/) and
...
//
// For example, the embedded path "templates/.claude/agents/ae/expert-backend.md"
// becomes ".claude/agents/ae/expert-backend.md" in the returned fs.FS.
//
// In production this fs.FS is passed to NewDeployer() to create a Deployer
// that writes templates to the project root during "ae init".
