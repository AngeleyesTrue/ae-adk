...
func TestSettingsTemplateStatusLine(t *testing.T) {
	...
	if sl["command"] != ".ae/status_line.sh" {
		t.Errorf("statusLine.command = %v, want %q", sl["command"], ".ae/status_line.sh")
	}
}
...
// "ae update hardcodes Linux PATH into settings.json, breaking MCP servers on macOS".
