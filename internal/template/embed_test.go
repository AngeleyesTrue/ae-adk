...
func TestEmbeddedTemplates_RuleFiles(t *testing.T) {
	...
	var ruleCount int
	_ = fs.WalkDir(fsys, ".claude/rules/ae", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return nil
		}
		if !d.IsDir() && strings.HasSuffix(path, ".md") {
...
func TestEmbeddedTemplates_OutputStyles(t *testing.T) {
	...
	entries, err := fs.ReadDir(fsys, ".claude/output-styles/ae")
	if err != nil {
...
func TestEmbeddedTemplates_LLMConfig(t *testing.T) {
	...
	for _, e := range entries {
		...
		data, err := fs.ReadFile(fsys, ".ae/llm-configs/"+e.Name())
		if err != nil {
