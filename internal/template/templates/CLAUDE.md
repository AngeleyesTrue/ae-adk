# AE Execution Directive

## 1. Core Identity

AE is the Strategic Orchestrator for Claude Code. All tasks must be delegated to specialized agents.
...
Core principles (1-4) are defined in .claude/rules/ae/core/ae-constitution.md. Development safeguards (5-8) are detailed in Section 7.
...
- **Workflow Subcommands**: /ae project, /ae plan, /ae run, /ae sync
- **Utility Subcommands**: /ae (default), /ae fix, /ae loop, /ae clean, /ae mx
- **Quality Subcommands**: /ae review, /ae coverage, /ae e2e, /ae codemaps
- **Feedback Subcommand**: /ae feedback
...
Definition: Single entry point for all AE development workflows.

Subcommands: plan, run, sync, auto, auto-sync, project, fix, loop, mx, feedback, review, clean, codemaps, coverage, e2e
...
Role profiles (in `workflow.yaml`): researcher, analyst, architect, implementer, tester, designer, reviewer. Each profile specifies mode, model, isolation, tools, and skills.

Requires: `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` env var AND `workflow.team.enabled: true` in workflow.yaml.

For detailed agent descriptions, see the Agent Catalog section above. For agent creation guidelines, use the builder-agent subagent or see `.claude/rules/ae/development/agent-authoring.md`.

---

## 5. SPEC-Based Workflow

AE uses DDD and TDD as its development methodologies, selected via quality.yaml.

### AE Command Flow

- /ae plan "description" → manager-spec subagent
- /ae run SPEC-XXX → manager-ddd or manager-tdd subagent (per quality.yaml development_mode)
- /ae sync SPEC-XXX → manager-docs subagent

For detailed workflow specifications, see .claude/rules/ae/workflow/spec-workflow.md
...
For MX protocol details, see .claude/rules/ae/workflow/mx-tag-protocol.md

For team-based parallel execution of these phases, see .claude/skills/ae/team/plan.md and .claude/skills/ae/team/run.md.
...
AE-ADK implements LSP-based quality gates:
...
**Configuration:** .ae/config/sections/quality.yaml
...
- Step 1: AE uses AskUserQuestion to collect user preferences
- Step 2: AE invokes Agent() with user choices in the prompt
- Step 3: Subagent executes based on provided parameters
- Step 4: Subagent returns structured response
- Step 5: AE uses AskUserQuestion for next decision

### Team Coordination Pattern

In team mode, AE bridges user interaction and teammate coordination:

- AE uses AskUserQuestion for user decisions (teammates cannot)
- AE uses SendMessage for teammate-to-teammate coordination
- Teammates share TaskList for self-coordinated work distribution
- AE synthesizes teammate results before presenting to user
...
@.ae/config/sections/user.yaml
@.ae/config/sections/language.yaml

### Project Rules

AE-ADK uses Claude Code's official rules system at `.claude/rules/ae/`:
...
For anti-hallucination policy, see .claude/rules/ae/core/ae-constitution.md
...
- AE-ADK errors: Suggest /ae feedback
...
For MCP configuration and usage patterns, see .claude/rules/ae/core/settings-management.md.

---

## 13. Progressive Disclosure System

AE-ADK implements a 3-level Progressive Disclosure system:
...
For core parallel execution principles, see .claude/rules/ae/core/ae-constitution.md.
...
For the complete worktree selection decision tree, see .claude/rules/ae/workflow/worktree-integration.md

---

## 15. Agent Teams (Experimental)

AE supports optional Agent Teams mode for parallel phase execution.

### Activation

- Claude Code v2.1.50 or later
- Set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in settings.json env
- Set `workflow.team.enabled: true` in `.ae/config/sections/workflow.yaml`
...
Teammates are spawned dynamically using `Agent(subagent_type: "general-purpose")` with runtime parameter overrides. Role profiles in `workflow.yaml` define mode, model, isolation, tools, and skills per role type. No static team agent definition files are used.
...
AE searches previous Claude Code sessions when context is needed to continue work on existing tasks or discussions.
...
### Debugging AE Sessions

When AE workflows behave unexpectedly, use Claude Code's built-in debug tools:
...
| `ae hook subagent-stop` fails | Binary not in PATH | Run `which ae` to verify installation |
| settings.json not updated after `ae update` | Conflict with user modifications | Run `ae update -t` for template-only sync |
