# AE Foundation Core Examples

실용적인 예시를 통해 AE Foundation Core의 6가지 핵심 원칙을 학습합니다.
...
```bash
# Phase 1: SPEC 생성
/ae:1-plan "JWT 기반 사용자 인증 시스템 구현"
...
**Output**:
```markdown
Phase 1 Result (.ae/specs/SPEC-001/spec.md):
==============================================
ID: SPEC-001
...
**Problem**: AE가 에이전트 위임 없이 직접 코드를 작성하거나 수정함

```python
# 잘못된 예시
def ae_direct_execution():
    # AE가 직접 파일 수정 - 절대 금지!
...
```bash
# 잘못된 예시
/ae:1-plan "feature"  # 30K 사용
/ae:2-run SPEC-001    # 이전 컨텍스트 유지 → 토큰 부족!
```
...
/ae:2-run SPEC-001    # 새로운 180K 예산으로 시작
```
