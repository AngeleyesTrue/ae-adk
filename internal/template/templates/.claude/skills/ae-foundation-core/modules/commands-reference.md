# Commands Reference - AE-ADK Core Commands

Purpose: Complete reference for AE-ADK's 6 core commands used in SPEC-First DDD workflow.

Last Updated: 2025-11-25
Version: 2.0.0

---

## Quick Reference (30 seconds)

AE-ADK provides 6 core commands for SPEC-First DDD execution:

| Command            | Purpose                | Phase         |
| ------------------ | ---------------------- | ------------- |
| `/ae:0-project`  | Project initialization | Setup         |
| `/ae:1-plan`     | SPEC generation        | Planning      |
| `/ae:2-run`      | DDD implementation     | Development   |
| `/ae:3-sync`     | Documentation sync     | Documentation |
| `/ae:9-feedback` | Feedback collection    | Improvement   |
| `/ae:99-release` | Production deployment  | Release       |

Required Workflow:
```
1. /ae:0-project # Initialize
2. /ae:1-plan "description" # Generate SPEC
3. /clear # Clear context (REQUIRED)
4. /ae:2-run SPEC-001 # Implement
5. /ae:3-sync SPEC-001 # Document
6. /ae:9-feedback # Improve
```

Critical Rule: Execute `/clear` after `/ae:1-plan` (saves 45-50K tokens)

---

## Implementation Guide (5 minutes)

### `/ae:0-project` - Project Initialization

Purpose: Initialize project structure and generate configuration

Agent Delegation: `workflow-project`

Usage:
```bash
/ae:0-project
/ae:0-project --with-git
```

What It Does:
1. Creates `.ae/` directory structure
2. Generates `config.json` with default settings
3. Initializes Git repository (if `--with-git` flag provided)
4. Sets up AE-ADK workflows

Output:
- `.ae/` directory
- `.ae/config/config.yaml`
- `.ae/state/` (empty, ready for session state)
- `.ae/logs/` (empty, ready for logging)

Next Step: Ready for SPEC generation via `/ae:1-plan`

Example:
```
User: /ae:0-project
AE: Project initialized successfully.
 - .ae/config/config.yaml created
 - Git workflow set to 'manual' mode
 Ready for SPEC generation.
```

---

### `/ae:1-plan` - SPEC Generation

Purpose: Generate SPEC document in EARS format

Agent Delegation: `workflow-spec`

Usage:
```bash
/ae:1-plan "Implement user authentication endpoint (JWT)"
/ae:1-plan "Add dark mode toggle to settings page"
```

What It Does:
1. Analyzes user request
2. Generates EARS format SPEC document
3. Creates `.ae/specs/SPEC-XXX/` directory
4. Saves `spec.md` with requirements

EARS Format (5 sections):
- WHEN (trigger conditions)
- IF (preconditions)
- THE SYSTEM SHALL (functional requirements)
- WHERE (constraints)
- UBIQUITOUS (quality requirements)

Output:
- `.ae/specs/SPEC-001/spec.md` (EARS document)
- SPEC ID assigned (auto-incremented)

CRITICAL: Execute `/clear` immediately after completion
- Saves 45-50K tokens
- Prepares clean context for implementation

Example:
```
User: /ae:1-plan "Implement user authentication endpoint (JWT)"
AE: SPEC-001 generated successfully.
 Location: .ae/specs/SPEC-001/spec.md

 IMPORTANT: Execute /clear now to free 45-50K tokens.
```

---

### `/ae:2-run` - DDD Implementation

Purpose: Execute ANALYZE-PRESERVE-IMPROVE cycle

Agent Delegation: `workflow-ddd`

Usage:
```bash
/ae:2-run SPEC-001
/ae:2-run SPEC-002
```

What It Does:
1. Reads SPEC document
2. Executes DDD cycle in 3 phases:
 - ANALYZE: Understand requirements and existing behavior
 - PRESERVE: Ensure existing behavior is protected with tests
 - IMPROVE: Implement improvements incrementally
3. Validates TRUST 5 quality gates
4. Generates implementation report

DDD Process:
```
Phase 1 (ANALYZE):
 - Understand requirements from SPEC
 - Analyze existing codebase behavior
 - Identify areas of change

Phase 2 (PRESERVE):
 - Create characterization tests for existing behavior
 - Ensure all tests pass before changes
 - Run tests → ALL PASS

Phase 3 (IMPROVE):
 - Implement changes incrementally
 - Validate behavior preservation
 - Optimize code structure
 - Run tests → ALL PASS (maintained)
```

Output:
- Implemented code (in source directories)
- Test files (in test directories)
- Quality report (TRUST 5 validation)

Requirement: Test coverage ≥ 85% (TRUST 5)

Example:
```
User: /ae:2-run SPEC-001
AE: DDD implementation cycle started for SPEC-001.

 ANALYZE: Requirements analyzed, 12 acceptance criteria identified
 PRESERVE: Existing behavior protected, characterization tests created
 IMPROVE: Implementation complete, all tests passing

 Test Coverage: 92% ( meets 85% threshold)
 TRUST 5: All gates passed
```

---

### `/ae:3-sync` - Documentation Synchronization

Purpose: Auto-generate API documentation and project artifacts

Agent Delegation: `workflow-docs`

Usage:
```bash
/ae:3-sync SPEC-001
/ae:3-sync SPEC-002
```

What It Does:
1. Reads implemented code
2. Generates API documentation (OpenAPI format)
3. Creates architecture diagrams
4. Produces project completion report

Output:
- API documentation (OpenAPI/Swagger format)
- Architecture diagrams (Mermaid)
- `.ae/docs/SPEC-001/` directory
- Project report

Example:
```
User: /ae:3-sync SPEC-001
AE: Documentation synchronized for SPEC-001.

 Generated:
 - API documentation: .ae/docs/SPEC-001/api.yaml
 - Architecture diagram: .ae/docs/SPEC-001/architecture.md
 - Completion report: .ae/docs/SPEC-001/report.md
```

---

### `/ae:9-feedback` - Improvement Feedback Collection

Purpose: Error analysis and improvement suggestions

Agent Delegation: `core-quality`

Usage:
```bash
/ae:9-feedback
/ae:9-feedback --analyze SPEC-001
```

What It Does:
1. Analyzes errors encountered during workflow
2. Collects improvement suggestions
3. Reports to AE-ADK development team
4. Proposes error recovery strategies

Use Cases:
- Errors: When errors occur during any workflow phase
- Improvements: When AE-ADK enhancements are identified
- Analysis: Post-implementation review

Example:
```
User: /ae:9-feedback
AE: Collecting feedback for recent session.

 Errors: 2 permission issues detected
 Improvements: 1 token optimization suggestion

 Feedback submitted to AE-ADK development team.
```

---

### `/ae:99-release` - Production Deployment

Purpose: Production deployment workflow

Agent Delegation: `infra-devops`

Usage:
```bash
/ae:99-release
```

What It Does:
1. Validates all TRUST 5 quality gates
2. Runs full test suite
3. Builds production artifacts
4. Deploys to production environment

Note: This command is local-only and NOT synchronized to the package template. It's for local development and testing.

---

## Advanced Implementation (10+ minutes)

### Context Initialization Rules

Rule 1: Execute `/clear` AFTER `/ae:1-plan` (mandatory)
- SPEC generation uses 45-50K tokens
- `/clear` frees this context for implementation phase
- Prevents context overflow

Rule 2: Execute `/clear` when context > 150K tokens
- Monitor context usage via `/context` command
- Prevents token limit exceeded errors

Rule 3: Execute `/clear` after 50+ conversation messages
- Accumulated context from conversation history
- Reset for fresh context

Why `/clear` is critical:
```
Without /clear:
 SPEC generation: 50K tokens
 Implementation: 100K tokens
 Total: 150K tokens (approaching 200K limit)

With /clear:
 SPEC generation: 50K tokens
 /clear: 0K tokens (reset)
 Implementation: 100K tokens
 Total: 100K tokens (50K budget remaining)
```

---

### Command Delegation Patterns

Each command delegates to a specific agent:

| Command            | Agent              | Agent Type              |
| ------------------ | ------------------ | ----------------------- |
| `/ae:0-project`  | `workflow-project` | Tier 1 (Always Active)  |
| `/ae:1-plan`     | `workflow-spec`    | Tier 1 (Always Active)  |
| `/ae:2-run`      | `workflow-ddd`     | Tier 1 (Always Active)  |
| `/ae:3-sync`     | `workflow-docs`    | Tier 1 (Always Active)  |
| `/ae:9-feedback` | `core-quality`     | Tier 2 (Auto-triggered) |
| `/ae:99-release` | `infra-devops`     | Tier 3 (Lazy-loaded)    |

Delegation Flow:
```
User executes command
 ↓
AE receives command
 ↓
Command processor agent invoked
 ↓
Agent executes workflow
 ↓
Results reported to user
```

---

### Token Budget by Command

| Command        | Average Tokens | Phase Budget                          |
| -------------- | -------------- | ------------------------------------- |
| `/ae:1-plan` | 45-50K         | Planning Phase (30K allocated)        |
| `/ae:2-run`  | 80-100K        | Implementation Phase (180K allocated) |
| `/ae:3-sync` | 20-25K         | Documentation Phase (40K allocated)   |
| Total          | 145-175K       | 250K per feature                      |

Optimization:
- Use Haiku 4.5 for `/ae:2-run` (fast, cost-effective)
- Use Sonnet 4.5 for `/ae:1-plan` (high-quality SPEC)
- Execute `/clear` between phases (critical)

---

### Error Handling

Common Errors:

| Error                     | Command                | Solution                                    |
| ------------------------- | ---------------------- | ------------------------------------------- |
| "Project not initialized" | `/ae:1-plan`         | Run `/ae:0-project` first                 |
| "SPEC not found"          | `/ae:2-run SPEC-999` | Verify SPEC ID exists                       |
| "Token limit exceeded"    | Any                    | Execute `/clear` immediately                |
| "Test coverage < 85%"     | `/ae:2-run`          | `core-quality` auto-generates missing tests |

Recovery Pattern:
```bash
# Error: Token limit exceeded
1. /clear # Reset context
2. /ae:2-run SPEC-001 # Retry with clean context
```

---

### Workflow Variations

Standard Workflow (Full SPEC):
```
/ae:0-project → /ae:1-plan → /clear → /ae:2-run → /ae:3-sync
```

Quick Workflow (No SPEC for simple tasks):
```
/ae:0-project → Direct implementation (for 1-2 file changes)
```

Iterative Workflow (Multiple SPECs):
```
/ae:1-plan "Feature A" → /clear → /ae:2-run SPEC-001 → /ae:3-sync SPEC-001
/ae:1-plan "Feature B" → /clear → /ae:2-run SPEC-002 → /ae:3-sync SPEC-002
```

---

### Integration with Git Workflow

Commands automatically integrate with Git based on `config.json` settings:

Manual Mode (Local Git):
- `/ae:1-plan`: Prompts for branch creation
- `/ae:2-run`: Auto-commits to local branch
- No auto-push

Personal Mode (GitHub Individual):
- `/ae:1-plan`: Auto-creates feature branch + auto-push
- `/ae:2-run`: Auto-commits + auto-push
- `/ae:3-sync`: Suggests PR creation (user choice)

Team Mode (GitHub Team):
- `/ae:1-plan`: Auto-creates feature branch + Draft PR
- `/ae:2-run`: Auto-commits + auto-push
- `/ae:3-sync`: Prepares PR for team review

---

## Works Well With

Skills:
- [ae-foundation-core](../SKILL.md) - Parent skill
- [ae-foundation-context](../../ae-foundation-context/SKILL.md) - Token budget management

Other Modules:
- [spec-first-ddd.md](spec-first-ddd.md) - Detailed SPEC-First DDD process
- [token-optimization.md](token-optimization.md) - /clear execution strategies
- [agents-reference.md](agents-reference.md) - Agent catalog

Agents:
- [workflow-project](agents-reference.md#tier-1-command-processors) - `/ae:0-project`
- [workflow-spec](agents-reference.md#tier-1-command-processors) - `/ae:1-plan`
- [workflow-ddd](agents-reference.md#tier-1-command-processors) - `/ae:2-run`
- [workflow-docs](agents-reference.md#tier-1-command-processors) - `/ae:3-sync`

---

Maintained by: AE-ADK Team
Status: Production Ready
