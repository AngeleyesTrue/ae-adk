...
Purpose: Security policies, execution constraints, and Git workflow strategies governing AE-ADK agent behavior.
...
AE operates under strict execution rules:
...
| `Skill()` | Knowledge invocation | `Skill("ae-foundation-core")` |
...
AE automatically adjusts Git workflow based on `config.json` settings.
...
AE's Behavior (prompt_always=true):
1. When running `/ae:1-plan`, user prompted: "Create branch?"
...
AE's Behavior (prompt_always=false):
...
2. Running `/ae:2-run`: DDD commits + auto push
3. Running `/ae:3-sync`: Doc commits + suggest PR creation (user choice)
...
AE's Behavior (prompt_always=false, auto_enabled=false):
1. When running `/ae:1-plan`, user prompted once: "Enable automatic branch creation?"
...
AE's Behavior (prompt_always=false, auto_enabled=true):
...
2. Running `/ae:2-run`: DDD commits + auto push (to feature branch)
3. Running `/ae:3-sync`: Doc commits + prepare PR
...
AE's Behavior (prompt_always=false, auto_enabled=false):
1. When running `/ae:1-plan`, user prompted once: "Enable automatic branch creation and Draft PR creation?"
...
Execute `/clear` after `/ae:1-plan` completion in ALL modes.
...
1. Immediately after `/ae:1-plan` (mandatory)
...
- Create documentation in `.ae/` directory
...
This comprehensive set of execution rules ensures that AE-ADK operates securely, efficiently, and in compliance with industry standards while maintaining high quality output.
