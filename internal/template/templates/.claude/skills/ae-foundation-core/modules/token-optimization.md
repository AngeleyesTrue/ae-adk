...
1. Immediately after /ae:1-plan (saves 45-50K)
...
def restore_minimal_context(summary: dict):
 ...
 load_file(f".ae/specs/{summary['current_spec']}/spec.md")
...
class SelectiveFileLoader:
 ...
 f".ae/specs/{context['spec_id']}/spec.md"
 ]
 ...
 files = [f".ae/specs/{context['spec_id']}/spec.md"]
...
def load_history():
 load_file(".ae/conversation_history.json") # 500K+ tokens
```
...
Skills:
- ae-foundation-delegation-patterns - Context passing
- ae-foundation-progressive-disclosure - Content structuring
- ae-cc-memory - Context persistence

Commands:
- /clear - Context reset (mandatory after /ae:1-plan)
- /context - Check current token usage
- /ae:1-plan - SPEC generation (30K budget)
- /ae:2-run - DDD implementation (180K budget)
- /ae:3-sync - Documentation (40K budget)

Memory:
- Skill("ae-foundation-core") modules/token-optimization.md - Optimization strategies
- .ae/config/config.json - Budget configuration
