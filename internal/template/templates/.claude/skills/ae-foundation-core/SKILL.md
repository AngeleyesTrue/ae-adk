---
name: ae-foundation-core
description: >
  Provides AE-ADK foundational principles including TRUST 5 quality framework,
  ...
  agent delegation patterns, or AE orchestration rules.
  Do NOT use for context and token management (use ae-foundation-context instead)
  or strategic analysis (use ae-foundation-philosopher instead).
license: Apache-2.0
...
Foundational principles and architectural patterns that power AE-ADK's AI-driven development workflow.
...
Phase 1 SPEC (/ae:1-plan): workflow-spec generates EARS format. Output is .ae/specs/SPEC-XXX/spec.md. Execute /clear to save 45-50K tokens.

Phase 2 DDD (/ae:2-run): ANALYZE for requirements, PRESERVE for existing behavior, IMPROVE for enhancement. Validate with at least 85% coverage.

Phase 3 Docs (/ae:3-sync): API documentation, architecture diagrams, project reports.
...
Core Principle: AE must delegate all work through Agent() to specialized agents. Direct execution bypasses specialization, quality gates, and token optimization. Proper delegation improves task success rate by 40 percent and enables parallel execution.
...
Phase Separation: Execute /clear between phases, after /ae:1-plan to save 45-50K, when context exceeds 150K, after 50+ messages.
...
Skills: ae-cc-claude-md for CLAUDE.md with foundation patterns, ae-cc-configuration for config with TRUST 5, ae-cc-memory for token optimization, ae-context7-integration for MCP integration.

Tools: AskUserQuestion for direct user interaction and clarification needs.

Commands: /ae:1-plan for SPEC-First Phase 1, /ae:2-run for DDD Phase 2, /ae:3-sync for Documentation Phase 3, /ae:9-feedback for continuous improvement, /clear for token management.
