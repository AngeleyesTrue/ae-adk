# AE-ADK Integration Module

Purpose: Detailed integration patterns for ae-worktree with AE-ADK Plan-Run-Sync workflow including plan phase automation, DDD integration, and cleanup workflows.
...
AE-ADK Integration Points:
- /ae:1-plan: Automatic worktree creation after SPEC generation
- /ae:2-run: DDD execution in isolated worktree environment
- /ae:3-sync: Worktree sync with documentation updates
- Cleanup: Automatic removal of merged worktrees

---

## Plan Phase Integration (/ae:1-plan)
...
- Default: {repo}/.ae/worktrees/{project-name}/SPEC-{id}
- Configurable via worktree_root setting

Creation Workflow:
1. SPEC is created with /ae:1-plan
...
## Development Phase Integration (/ae:2-run)
...
- Looks for .ae/worktrees directory in path hierarchy
...
## Sync Phase Integration (/ae:3-sync)
...
### AE Configuration Integration

Worktree settings in .ae/config/config.yaml:
...
- worktree_root: Base directory for worktrees (default: {repo}/.ae/worktrees)
...
Version: 1.0.0
Last Updated: 2026-01-06
Module: AE-ADK workflow integration patterns for Plan-Run-Sync phases
