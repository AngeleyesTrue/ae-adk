...
- Integration Issues: AE workflow integration and command coordination

Quick Diagnostics:
- Check worktree status: ae-worktree status --all
- Verify registry integrity: ae-worktree registry validate
...
Prevention:
- Always use ae-worktree remove instead of manual directory deletion
...
Symptoms:
- ae-worktree commands fail with JSON parsing errors
...
- Validate the rebuilt registry with ae-worktree status --all

### Orphaned Registry Entries

Symptoms:
- Registry lists worktrees that no longer exist on disk
- ae-worktree status shows worktrees as missing
...
Resolution Steps:
1. Run ae-worktree status --all to identify orphaned entries
2. For each orphaned entry, confirm the directory truly does not exist
3. Remove orphaned entries from registry using ae-worktree registry prune
...
- Use ae-worktree commands for standard operations
...
### AE Command Coordination Failures

Symptoms:
- /ae:1-plan does not create expected worktree
- /ae:2-run cannot find worktree for SPEC
- Workflow commands operate on wrong worktree

Root Causes:
- SPEC ID not matching worktree ID format
- Worktree was created manually without AE integration
- Configuration mismatch between AE and worktree settings
- Working directory context not in expected location

Resolution Steps:
1. Verify SPEC ID format matches expected worktree naming convention
2. Ensure worktree was created through AE workflow or properly registered
3. Check ae configuration for worktree integration settings
...
Status Commands:
- ae-worktree status --all: Shows all worktrees with sync status
- git worktree list: Native Git worktree listing
- ae-worktree status SPEC-ID --detailed: Detailed status for specific worktree

Registry Commands:
- ae-worktree registry validate: Checks registry integrity
- ae-worktree registry prune: Removes orphaned entries
- ae-worktree registry export: Exports registry for backup
...
Cleanup Commands:
- ae-worktree clean --dry-run: Preview cleanup without changes
- ae-worktree clean --merged-only: Clean only merged worktrees
- git worktree prune: Remove stale Git worktree metadata

Recovery Commands:
- ae-worktree registry rebuild: Reconstruct registry from directories
- ae-worktree remove SPEC-ID --keep-branch: Remove worktree, preserve branch
...
Weekly Tasks:
- Run ae-worktree status --all to check for issues
...
Creation:
- Always use ae-worktree new instead of manual Git commands
...
Removal:
- Use ae-worktree remove instead of manual deletion
