...
Quick Start:
1. Create worktree: ae-worktree new SPEC-001 "User Authentication"
2. Switch to worktree: ae-worktree switch SPEC-001
3. Or use shell eval: eval $(ae-worktree go SPEC-001)

---

## Creation Commands

### ae-worktree new - Create Worktree

Create a new isolated Git worktree for SPEC development.

Syntax: ae-worktree new <spec-id> [description] [options]
...
Examples:
- Basic creation: ae-worktree new SPEC-001 "User Auth System"
- Custom branch: ae-worktree new SPEC-002 "Payment" --branch feature/payment-gateway
- From develop: ae-worktree new SPEC-003 "API Refactor" --base develop
- With template: ae-worktree new SPEC-004 "Frontend" --template frontend
- Fast creation: ae-worktree new SPEC-005 "Bug Fixes" --shallow --depth 1
...
Examples:
- Table format: ae-worktree list
- JSON output: ae-worktree list --format json
- Active only: ae-worktree list --status active
- Sort by date: ae-worktree list --sort created
- Detailed: ae-worktree list --verbose

### ae-worktree switch - Switch to Worktree

Change current working directory to the specified worktree.

Syntax: ae-worktree switch <spec-id> [options]
...
Examples:
- Basic switch: ae-worktree switch SPEC-001
- With sync: ae-worktree switch SPEC-002 --auto-sync
- Force switch: ae-worktree switch SPEC-003 --force

### ae-worktree go - Get Worktree Path

Output the cd command for shell integration.

Syntax: ae-worktree go <spec-id> [options]
...
- eval pattern (recommended): eval $(ae-worktree go SPEC-001)
- source pattern: ae-worktree go SPEC-001 | source
- manual cd: cd $(ae-worktree go SPEC-001 --absolute)

---

## Management Commands

### ae-worktree sync - Synchronize Worktree

Synchronize worktree with its base branch.

Syntax: ae-worktree sync <spec-id> [options]
...
Examples:
- Sync specific: ae-worktree sync SPEC-001
- Sync all: ae-worktree sync --all
- Interactive: ae-worktree sync SPEC-001 --interactive
- Preview: ae-worktree sync SPEC-001 --dry-run
- Include pattern: ae-worktree sync SPEC-001 --include "src/"
- Exclude pattern: ae-worktree sync SPEC-001 --exclude "node_modules/"
...
### ae-worktree remove - Remove Worktree

Remove a worktree and clean up its registration.

Syntax: ae-worktree remove <spec-id> [options]
...
Examples:
- Interactive: ae-worktree remove SPEC-001
- Force: ae-worktree remove SPEC-001 --force
- Keep branch: ae-worktree remove SPEC-001 --keep-branch
- With backup: ae-worktree remove SPEC-001 --backup
- Preview: ae-worktree remove SPEC-001 --dry-run

### ae-worktree clean - Clean Up Worktrees

Remove worktrees for merged branches or stale worktrees.

Syntax: ae-worktree clean [options]
...
Examples:
- Merged only: ae-worktree clean --merged-only
- Stale (30 days): ae-worktree clean --stale
- Custom threshold: ae-worktree clean --stale --days 14
- Interactive: ae-worktree clean --interactive
- Preview: ae-worktree clean --dry-run
- Force: ae-worktree clean --force

---

## Status and Configuration

### ae-worktree status - Show Worktree Status

Display detailed status information about worktrees.

Syntax: ae-worktree status [spec-id] [options]
...
Examples:
- Current worktree: ae-worktree status
- Specific worktree: ae-worktree status SPEC-001
- All with sync check: ae-worktree status --all --sync-check
- Detailed Git status: ae-worktree status SPEC-001 --detailed
- JSON output: ae-worktree status --all --format json
...
### ae-worktree config - Configuration Management

Manage ae-worktree configuration settings.

Syntax: ae-worktree config <action> [key] [value]
...
Examples:
- List all: ae-worktree config list
- Get value: ae-worktree config get worktree_root
- Set value: ae-worktree config set auto_sync true
- Reset: ae-worktree config reset worktree_root
- Edit: ae-worktree config edit
...
- ae-worktree clean --merged-only --force
