...
To set up parallel development, create worktrees for each SPEC using ae-worktree new with the SPEC ID and description. For parallel execution, navigate to each worktree using ae-worktree go and run /ae:2-run in separate terminals or background processes. For integration, use ae-worktree sync --all to synchronize all worktrees, then clean up merged worktrees with ae-worktree clean --merged-only.
...
Worktree Root ({repo}/.ae/worktrees/{project-name}/):
