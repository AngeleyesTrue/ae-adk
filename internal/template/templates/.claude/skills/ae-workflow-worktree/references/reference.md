# AE Worktree Reference

Purpose: External resources, documentation, and additional learning materials for ae-worktree skill.
...
- Click Framework: [https://click.palletsprojects.com/](https://click.palletsprojects.com/)
 - Command-line interface framework used by ae-worktree
...
- Rich Library: [https://rich.readthedocs.io/](https://rich.readthedocs.io/)
 - Terminal output formatting used by ae-worktree
...
- ae-foundation-core: Foundation principles and delegation patterns
 - Reference: [ae-foundation-core modules](ae-foundation-core/modules/)
 - Integration: Worktree creation follows foundation delegation patterns

- ae-workflow-project: Project management and configuration
...
1. Shallow Worktrees: For fast prototyping and testing
 ```bash
 ae-worktree new SPEC-PROTO-001 "Prototype" --shallow --depth 1
 ```

2. Selective Synchronization: Sync only essential files
 ```bash
 ae-worktree sync SPEC-001 --include "src/" --exclude "node_modules/"
 ```

3. Background Operations: Non-blocking worktree operations
 ```bash
 ae-worktree sync --all --background
 ```
...
jobs:
 ...
 ae-worktree new test-spec "Test Worktree"

 # Test worktree synchronization
 ae-worktree sync test-spec

 # Test worktree cleanup
 ae-worktree remove test-spec
```
...
pipeline {
 ...
 steps {
 sh '''
 ae-worktree new SPEC-AUTH-001 "Authentication Worktree"
 cd $(ae-worktree go SPEC-AUTH-001)
 npm test
 '''
 }
 }
 ...
 steps {
 sh '''
 ae-worktree new SPEC-PAY-001 "Payment Worktree"
 cd $(ae-worktree go SPEC-PAY-001)
 npm test
 '''
 }
 }
...
- Integration with AE-ADK workflow commands
...
git clone https://github.com/your-org/ae-adk.git
...
Version: 1.0.0
Last Updated: 2025-11-29
Reference: External resources and additional learning materials for ae-worktree
