# AE Worktree Examples

Purpose: Real-world usage examples and patterns for ae-worktree skill integration with AE-ADK workflow.
...
/ae:1-plan "User Authentication System" --worktree

# Output:
# SPEC created: SPEC-AUTH-001
# Worktree created: .ae/worktrees/AE-ADK/SPEC-AUTH-001
#
# Next steps:
# 1. Switch to worktree: ae-worktree switch SPEC-AUTH-001
# 2. Or use shell eval: eval $(ae-worktree go SPEC-AUTH-001)
# 3. Start development: /ae:2-run SPEC-AUTH-001

# Development Phase - Implement in isolated environment
eval $(ae-worktree go SPEC-AUTH-001)
/ae:2-run SPEC-AUTH-001

# Sync Phase - Synchronize and integrate
ae-worktree sync SPEC-AUTH-001
/ae:3-sync SPEC-AUTH-001

# Cleanup Phase - Remove completed worktree
ae-worktree remove SPEC-AUTH-001
```
...
ae-worktree new SPEC-AUTH-001 "User Authentication"
ae-worktree new SPEC-PAY-001 "Payment Processing"
ae-worktree new SPEC-DASH-001 "Dashboard Analytics"

# Switch between worktrees
ae-worktree switch SPEC-AUTH-001
# Work on authentication...

ae-worktree switch SPEC-PAY-001
# Work on payment system...

ae-worktree switch SPEC-DASH-001
# Work on dashboard...

# Check status of all worktrees
ae-worktree status --all

# Sync all worktrees
ae-worktree sync --all

# Clean up completed worktrees
ae-worktree clean --merged-only
```
...
```bash
# /ae:2-run automatically detects worktree environment
/ae:2-run SPEC-AUTH-001
...
ae-worktree status SPEC-AUTH-001
ae-worktree sync SPEC-AUTH-001 --include "src/"
ae-worktree config get worktree_root
```
...
/ae:1-plan "$SPEC_DESCRIPTION" --worktree --spec-id "$SPEC_ID"

# Check if worktree was created successfully
if ae-worktree list --format json | jq -r ".worktrees[\"$SPEC_ID\"]" > /dev/null; then
 echo " Worktree $SPEC_ID created successfully"

 # Phase 2: Develop
 echo " Phase 2: Switching to worktree for development..."
 cd $(ae-worktree go "$SPEC_ID")
...
WORKTREE_ROOT=".ae/worktrees/$PROJECT_NAME"
...
ae-worktree config set worktree_root "$WORKTREE_ROOT"
ae-worktree config set registry_type team

echo " Team worktree configuration completed"
echo "Team members can now join with: ae-worktree join --team $TEAM_NAME"
```
...
DEBUG=true""",
 ...
 # Save template
 template_path = Path.home() / ".ae-worktree/templates" / "fullstack.json"
...
# ae-worktree new SPEC-FULL-001 "Fullstack Application" --template fullstack
```
...
class WorktreeAutomation:
 ...
 "ae-worktree", "new", spec_id, description,
 "--template", template
 ...
 result = subprocess.run([
 "ae-worktree", "list", "--status", "active", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 "ae-worktree", "status", "--all", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 'action': 'ae-worktree clean --stale --days 30'
 })
 ...
 'action': 'ae-worktree optimize --analyze'
 })
...
ae-worktree new SPEC-DEBUG-001 "Debug Test" --verbose

# Problem 2: Worktree sync conflicts
echo " Resolving sync conflicts..."

# Check sync status
ae-worktree status SPEC-CONFLICT-001

# Interactive conflict resolution
ae-worktree sync SPEC-CONFLICT-001 --interactive

# Force sync (if appropriate)
ae-worktree sync SPEC-CONFLICT-001 --force

# Problem 3: Worktree registry corruption
echo " Repairing worktree registry..."

# Backup current registry
cp .ae/worktrees/PROJECT/.ae-worktree-registry.json .ae/worktrees/PROJECT/.ae-worktree-registry.json.backup

# Rebuild registry from worktree directories
ae-worktree config set registry_rebuild true
ae-worktree list --rebuild-registry

# Problem 4: Permission issues
echo " Fixing permission issues..."

# Check worktree permissions
ls -la .ae/worktrees/PROJECT/

# Fix permissions
chmod -R 755 .ae/worktrees/PROJECT/
```
...
Version: 1.0.0
Last Updated: 2025-11-29
Examples: Real-world usage patterns for ae-worktree integration
