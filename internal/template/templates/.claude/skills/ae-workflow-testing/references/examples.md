...
**Input**:
```python
from ae_workflow_testing import DDDManager, TestSpecification, TestType
...
**Input**:
```python
from ae_workflow_testing import AutomatedCodeReviewer
  ...
  "auto_fixable": [
    {
      "file": "src/auth/service.py",
      "fix": "Add type hints",
      "apply_command": "ae-workflow fix --file src/auth/service.py --type hints"
    }
  ]
}
```
...
```python
from ae_workflow_testing import QualityGateManager
...
```bash
# 전체 워크플로우 실행
ae-workflow execute --project /project/src --mode full

# 개별 컴포넌트 실행
ae-workflow debug --file app.py --error "AttributeError"
ae-workflow refactor --directory src/ --max-risk medium
ae-workflow profile --target function_name --types cpu,memory
ae-workflow test --spec user_auth.spec --mode ddd
ae-workflow review --project /project/src --trust-score-min 0.8

# CI 모드
ae-workflow ci --commit abc123 --quality-gates strict
```
