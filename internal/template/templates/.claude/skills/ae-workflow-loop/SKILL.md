...
- /ae: One-click Plan-Run-Sync automation (default)
- /ae loop: Start feedback loop
- /ae fix: One-time auto-fix
...
The Ralph Engine follows a layered architecture. User commands such as /ae:loop, /ae:fix, and /ae enter the Command Layer. The Command Layer invokes the Hook System, which contains the PostToolUse Hook for LSP diagnostics and the Stop Hook for loop control. The Hook System connects to Backend Services including the LSP Client (AELSPClient), AST-grep Scanner, and Test Runner. Backend Services feed into Completion Check which evaluates whether errors are zero, tests pass, and coverage is met. Based on the Completion Check result, the system either continues the loop or completes.
...
The Stop hook is triggered after each Claude response. The hook reads the loop state file located at .ae/cache/.ae_loop_state.json. This state contains active status (true or false), current iteration number, max_iterations limit, last_error_count from previous iteration, and completion_reason when finished.
...
The Go LSP client is integrated into the hook system. LSP diagnostics are automatically collected via the post-tool hook (ae hook post-tool-use).
...
For GitHub Actions integration, create a workflow step that runs Claude with the /ae:loop command and max-iterations flag. Set the MOAI_LOOP_ACTIVE environment variable to true to enable loop mode.
...
Review the max_iterations setting to ensure it allows sufficient iterations. Review completion conditions to verify they are achievable. Send any message to interrupt the loop, or delete the state file (.ae/cache/.ae_loop_state.json) to reset.

## Works Well With

Skills:

- ae-foundation-quality: TRUST 5 validation
- ae-tool-ast-grep: Security scanning patterns
- ae-workflow-testing: DDD integration
- ae-lang-python: Python-specific patterns
- ae-lang-typescript: TypeScript patterns
...
Configuration is stored at .ae/config/sections/ralph.yaml.

Loop state is stored at .ae/cache/.ae_loop_state.json.

The LSP hook is located at .claude/hooks/ae/post_tool__lsp_diagnostic.

The loop hook is located at .claude/hooks/ae/stop__loop_controller.
