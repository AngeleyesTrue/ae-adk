...
| `ralph.ast_grep.config_path`                        | string  | `.claude/skills/ae-tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
> Note: The Go edition provides LSP capabilities through compiled hook subcommands in internal/lsp/. The examples below show the conceptual API; actual invocation is via ae hook post-tool-use.
...
```python
AELSPClient(project_root: str | Path)
```
...
client = AELSPClient(project_root="/path/to/project")
```
...
### Loop State File (.ae_loop_state.json)

Location: `.ae/cache/.ae_loop_state.json`
...
Location: `.claude/skills/ae-tool-ast-grep/rules/sgconfig.yml`
...
3. Check configuration file exists:
   ...
   ```bash
   ls .claude/skills/ae-tool-ast-grep/rules/sgconfig.yml
   ```
    ...
    steps:
      ...
      - name: Commit fixes
        ...
        run: |
          ...
          git config user.email "ralph@ae-adk.dev"
          git add .
...
```python
# .claude/hooks/ae/custom_completion_check
