...
/ae:loop --max-iterations 5

# Start with specific completion criteria
/ae:loop --zero-warnings true
```

**Loop State Management:**

```json
// .ae/cache/.ae_loop_state.json
{
  "active": true,
  "iteration": 1,
  "max_iterations": 10,
...
ralph:
  ...
  ast_grep:
    ...
    quality_scan: true
    custom_rules:
      - .ae/ast-grep/security/**/*.yml
      - .ae/ast-grep/quality/**/*.yml
...
ralph:
  ...
  ast_grep:
    enabled: true
    rules:
      - .ae/ast-grep/react/**/*.yml
      - .ae/ast-grep/typescript/**/*.yml
```
...
def load_loop_state() -> dict:
    state_file = Path(".ae/cache/.ae_loop_state.json")
...
```yaml
# .ae/ast-grep/security/sql-injection.yml
...
```yaml
# .ae/ast-grep/security/xss-prevention.yml
id: xss-react
language: typescript
rule:
...
```yaml
# .ae/ast-grep/security/hardcoded-secrets.yml
...
jobs:
  ralph-check:
    ...
    steps:
      ...
      - name: Run Ralph Loop
        run: |
          claude -p "/ae:loop --max-iterations 5" \
      ...
      - name: Check Results
        run: |
          python .ae/scripts/check_ralph_results.py
```
      ...
      - name: Run ${{ matrix.check }} Check
        run: |
          case "${{ matrix.check }}" in
            lsp)
              claude -p "/ae:fix --severity error" \
                --allowedTools "Read,Write,Edit"
              ;;
            ast-grep)
              ae ast-grep scan --security
              ;;
            tests)
              pytest tests/ -v
              ;;
...
# Run LSP diagnostics
ae lsp diagnose --changed-files

if [ $? -ne 0 ]; then
    echo "❌ LSP errors found. Run '/ae:fix' to resolve."
    exit 1
fi

# Run AST-grep security scan
ae ast-grep scan --security --changed-files
...
RUN claude -p "/ae:loop --max-iterations 3" \
    --allowedTools "Read,Write,Edit" \
...
```bash
# 1. Start development
/ae:1-plan "User authentication system"
...
```bash
# View LSP logs
cat .ae/logs/lsp_diagnostic.log

# Test LSP connection
ae lsp test-connection python

# View diagnostics directly
ae lsp diagnose src/auth.py
```
...
```bash
# View current state
cat .ae/cache/.ae_loop_state.json

# Reset loop state
rm .ae/cache/.ae_loop_state.json

# View loop logs
cat .ae/logs/loop_controller.log
```
