---
name: ae-workflow-spec
description: >
  SPEC workflow orchestration with EARS format requirements, acceptance criteria,
  and Plan-Run-Sync integration for AE-ADK development methodology.
  Use when creating SPEC documents, writing EARS requirements, defining acceptance
  criteria, planning features, or orchestrating the /ae plan phase.
  Do NOT use for implementation (use ae-workflow-ddd instead)
  or documentation generation (use ae-workflow-project instead).
license: Apache-2.0
...
metadata:
  ...
  updated: "2026-01-08"
  modularized: "true"
  tags: "workflow, spec, ears, requirements, ae-adk, planning"
  author: "AE-ADK Team"
  context: "fork"
  agent: "Plan"
...
- Create new SPEC: /ae:1-plan "user authentication system"
- Create parallel SPECs with Worktrees: /ae:1-plan "login feature" "signup feature" --worktree
- Create SPEC with new branch: /ae:1-plan "payment processing" --branch
- Update existing SPEC: /ae:1-plan SPEC-001 "add OAuth support"
...
Constitution defines the project DNA that all SPECs must respect. Before creating any SPEC, verify alignment with project constitution defined in `.ae/project/tech.md`.
...
- Command /ae:1-plan "login feature" "signup feature" --worktree creates multiple SPECs
...
- .ae/specs/SPEC-{ID}/: SPEC document directory containing 3 required files
...
- .ae/state/: Session state files (last-session-state.json)
- .ae/docs/: Generated documentation (api-documentation.md)
...
- ae-foundation-core: SPEC-First DDD methodology and TRUST 5 framework
- ae-workflow-testing: DDD implementation and test automation
- ae-workflow-project: Project initialization and configuration
- ae-workflow-worktree: Git Worktree management for parallel development
...
- Step 1 PLAN: /ae:1-plan "user authentication system"
- Step 2 RUN: /ae:2-run SPEC-001
- Step 3 SYNC: /ae:3-sync SPEC-001

Parallel Workflow:

- Create multiple SPECs: /ae:1-plan "backend API" "frontend UI" "database schema" --worktree
- Session 1: /ae:2-run SPEC-001 (backend API)
- Session 2: /ae:2-run SPEC-002 (frontend UI)
- Session 3: /ae:2-run SPEC-003 (database schema)
...
- SPEC document persists in .ae/specs/ directory
- Session state in .ae/state/ for cross-session context
...
### What Belongs in .ae/specs/

The `.ae/specs/` directory is EXCLUSIVELY for SPEC documents that define features to be implemented.
...
### What Does NOT Belong in .ae/specs/

| Document Type         | Why Not SPEC                  | Correct Location                          |
| --------------------- | ----------------------------- | ----------------------------------------- |
| Security Audit        | Analyzes existing code        | `.ae/reports/security-audit-{DATE}/`    |
| Performance Report    | Documents current metrics     | `.ae/reports/performance-{DATE}/`       |
| Dependency Analysis   | Reviews existing dependencies | `.ae/reports/dependency-review-{DATE}/` |
| Architecture Overview | Documents current state       | `.ae/docs/architecture.md`              |
| API Reference         | Documents existing APIs       | `.ae/docs/api-reference.md`             |
| Meeting Notes         | Records decisions made        | `.ae/reports/meeting-{DATE}/`           |
| Retrospective         | Analyzes past work            | `.ae/reports/retro-{DATE}/`             |

### Exclusion Rules

[HARD] Report vs SPEC Distinction:

Reports analyze what EXISTS → `.ae/reports/`
SPECs define what will be BUILT → `.ae/specs/`

[HARD] Documentation vs SPEC Distinction:

Documentation explains HOW TO USE → `.ae/docs/`
SPECs define WHAT TO BUILD → `.ae/specs/`
