...
Problem: `.ae/specs/SPEC-AUTH-001.md` exists as single file

Solution Steps:

1. Create directory: `mkdir -p .ae/specs/SPEC-AUTH-001/`
2. Move content: `mv .ae/specs/SPEC-AUTH-001.md .ae/specs/SPEC-AUTH-001/spec.md`
...
find .ae/specs -maxdepth 1 -name "SPEC-*.md" -type f
```
...
1. Find next available number:
   ```bash
   ls -d .ae/specs/SPEC-*-[0-9][0-9][0-9] 2>/dev/null | sort -t- -k3 -n | tail -1
   ```
2. Assign number: `SPEC-REDESIGN` → `SPEC-REDESIGN-001`
3. Rename directory:
   ```bash
   mv .ae/specs/SPEC-REDESIGN .ae/specs/SPEC-REDESIGN-001
   ```
...
Problem: Analysis/audit document in `.ae/specs/`
...
2. Create reports directory:
   ```bash
   mkdir -p .ae/reports/security-audit-2025-01/
   ```
3. Move content:
   ```bash
   mv .ae/specs/SPEC-SECURITY-AUDIT/* .ae/reports/security-audit-2025-01/
   rmdir .ae/specs/SPEC-SECURITY-AUDIT
   ```
...
1. Compare creation dates:
   ```bash
   ls -la .ae/specs/ | grep SPEC-AUTH-001
   ```
2. Determine which is canonical (usually older one)
3. Renumber newer one to next available:
   ```bash
   mv .ae/specs/SPEC-AUTH-001-duplicate .ae/specs/SPEC-AUTH-002
   ```
...
ls -d .ae/specs/SPEC-*/ 2>/dev/null | grep -v -E 'SPEC-[A-Z]+-[0-9]{3}'

# Check 4: Potential reports in specs
echo -e "\n[Check 4] Potential reports in specs (check manually):"
grep -l -r "findings\|recommendations\|audit\|analysis" .ae/specs/*/spec.md 2>/dev/null
