...
description: >
  ...
  (use ae-foundation-core instead).
license: Apache-2.0
...
metadata:
  ...
  aliases: "ae-foundation-context"
  replaces: "ae-core-context-budget, ae-core-session-state"
...
Mandatory /clear Points: After /ae:1-plan completion to save 45-50K tokens. When context exceeds 150K tokens to prevent overflow. When conversation exceeds 50 messages to remove stale history. Before major phase transitions for clean slate. During model switches for Haiku to Sonnet handoffs.
...
- ae-cc-memory - Memory management and context persistence
- ae-cc-configuration - Session configuration and preferences
- ae-core-workflow - Workflow state persistence and recovery
- ae-cc-agents - Agent state management across sessions
- ae-foundation-trust - Quality gate integration
...
SPEC-First Workflow: Execute /ae:1-plan, then mandatory /clear to save 45-50K tokens, then /ae:2-run SPEC-XXX, then multi-agent handoffs with Pattern 4, then /ae:3-sync SPEC-XXX, then session state persistence with Pattern 3.
