...
metadata:
  ...
  status: "active"
  updated: "2026-02-23"
  tags: "ae, autonomous, pipeline, plan-run-sync, default"
...
triggers:
  keywords: ["ae", "autonomous", "pipeline", "build", "implement", "create"]
  agents: ["ae"]
  phases: ["plan", "run", "sync"]
---

# Workflow: AE - Autonomous Development Orchestration

Purpose: Full autonomous workflow. User provides a goal, AE autonomously executes plan -> run -> sync pipeline. This is the default workflow when no subcommand is specified.

Flow: Explore -> Plan -> Run -> Sync -> Done

For phase overview, token budgets, and phase transitions, see: .claude/rules/ae/workflow/spec-workflow.md
...
[HARD] Before Phase 2 implementation, ALWAYS check `.ae/config/sections/quality.yaml`:
...
For methodology details, see: .claude/rules/ae/workflow/workflow-modes.md
...
- If .ae/project/codemaps/ exists: Use as architecture baseline to accelerate exploration (skip redundant scanning)
...
- Save research.md to .ae/specs/SPEC-{ID}/research.md when SPEC ID is determined
...
- Output: EARS-format SPEC document at .ae/specs/SPEC-XXX/spec.md
...
[HARD] Methodology selection based on `.ae/config/sections/quality.yaml`:
...
- Conditionally update project documents (.ae/project/) when structural changes detected
...
11.5. **Execution Mode Selection Gate**: After Phase 1.5 approval, before Phase 2
   ...
   - Otherwise (no flag):
     - Read .ae/config/sections/llm.yaml → team_mode ("" = cc, "glm" = glm, "cg" = cg)
...
Version: 2.9.0
Updated: 2026-04-09
Source: SPEC-AE-001. Fix --team/--solo flag Gate auto-skip (v2.9.0). Previous: Harness auto-detection (v2.8.0).
