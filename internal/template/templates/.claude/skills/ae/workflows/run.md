...
For methodology details (DDD ANALYZE-PRESERVE-IMPROVE and TDD RED-GREEN-REFACTOR cycles, success criteria, brownfield enhancement), see: .claude/rules/ae/workflow/workflow-modes.md
...
- Resume: Re-running /ae run SPEC-XXX resumes from last successful phase checkpoint
...
- .ae/config/config.yaml (git strategy, automation settings)
- .ae/config/sections/quality.yaml (coverage targets, TRUST 5 settings, development_mode)
...
- .ae/specs/SPEC-{ID}/ directory (spec.md, plan.md, acceptance.md)
- .ae/specs/SPEC-{ID}/progress.md (session resume context: if exists, load to identify completed phases and skip them; if absent, will be created at Phase 1 start)
- .ae/project/structure.md (architecture context for implementation decisions)
- .ae/project/tech.md (technology stack context)
- .ae/project/codemaps/ directory listing (architecture maps for dependency and module understanding)

Pre-execution commands: git status, git branch, git log, git diff.

### Resume Check

Before Phase 1, check if `.ae/specs/SPEC-{ID}/progress.md` exists:
...
Input: SPEC document content from the provided SPEC-ID. If research.md exists in the SPEC directory (.ae/specs/SPEC-{ID}/research.md), include it as additional context for deeper understanding of the codebase architecture, reference implementations, and identified risks.
...
Progress update: Append to `.ae/specs/SPEC-{ID}/progress.md`:
```
...
See .claude/rules/ae/workflow/mx-tag-protocol.md for tag type definitions.

### Batch Mode Decision [MANDATORY EVALUATION]

Before routing to Phase 2, AE MUST evaluate whether to use Skill("batch") for parallel implementation.
...
- If ANY condition is met: Execute Skill("batch") directly. Batch mode replaces sequential Phase 2. Each batch unit executes one atomic task in an isolated git worktree, runs tests, and creates a PR. AE collects all PRs and merges in dependency order via manager-git.
...
Before Phase 2, determine the development methodology by reading `.ae/config/sections/quality.yaml`:
...
Input: Approved execution plan from Phase 1 plus task decomposition from Phase 1.5. Include `.ae/project/structure.md` and `.ae/project/tech.md` as onboarding context in the agent prompt so the implementation agent understands the project's architecture conventions before writing code.
...
This divergence data is consumed by /ae sync for SPEC document updates and project document synchronization.
...
Purpose: Detect stagnation and trigger re-assessment if implementation is stuck. See .claude/rules/ae/workflow/spec-workflow.md for trigger conditions, communication path, and detection method.

Check `.ae/specs/SPEC-{ID}/progress.md` for stagnation signals. If triggered, return structured stagnation report to AE for user escalation.
...
Purpose: Update @MX code annotations for modified files. See .claude/rules/ae/workflow/mx-tag-protocol.md for tag rules.
...
Action: AE MUST call Skill("simplify") at this phase. Do not delegate to a subagent. Call it directly.
...
When the run phase is invoked from plan.md Decision Point 3.5 or ae.md Phase 11.5, the gate passes these parameters:
...
**If no execution_mode provided (direct `/ae run` invocation):**
...
See .claude/rules/ae/workflow/worktree-integration.md for the complete worktree decision tree.
...
- Phase 2 to /ae sync: Implementation divergence report enables accurate SPEC and project document updates
