...
- Marker types: `<ae>DONE</ae>`, `<ae>COMPLETE</ae>`
...
- If session duration exceeds 25 minutes OR iteration time doubling:
  - Save proactive checkpoint to $CLAUDE_PROJECT_DIR/.ae/cache/loop-snapshots/memory-pressure.json
  - Warn user about memory pressure
  - Suggest resuming with /ae:loop --resume memory-pressure
...
- See .claude/rules/ae/workflow/mx-tag-protocol.md for tag rules

Step 8 - Snapshot Save:
- Save iteration snapshot to $CLAUDE_PROJECT_DIR/.ae/cache/loop-snapshots/
...
Snapshot location: $CLAUDE_PROJECT_DIR/.ae/cache/loop-snapshots/
...
Loop state file: $CLAUDE_PROJECT_DIR/.ae/cache/.ae_loop_state.json

Resume commands:
- /ae:loop --resume latest
