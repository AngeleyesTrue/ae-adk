---
name: ae-workflow-auto-sync
description: >
  Pipeline-safe synchronization workflow without merge capability.
  Used by auto pipeline to prevent unintended merge operations.
  Synchronizes documentation with code changes, verifies project quality,
  and finalizes pull requests. Includes deep code review with auto-fix,
  coverage analysis with test generation, SPEC divergence analysis,
  project document updates, and Context Memory generation.
...
metadata:
  ...
  status: "active"
  updated: "2026-04-16"
  tags: "sync, documentation, pull-request, quality, verification, pr, context-memory, auto, pipeline"
...
triggers:
  keywords: ["auto-sync", "auto sync", "pipeline sync"]
  agents: ["manager-docs", "manager-quality", "manager-git"]
  phases: ["sync"]
---

# Auto-Sync Workflow Orchestration

## Pipeline Safety Constraint [HARD]

[HARD] This workflow has NO merge capability. It does NOT contain `gh pr merge`, `--merge` flag handling, or any auto-merge functionality. This is by design — merge operations are controlled exclusively by the auto pipeline orchestrator (auto.md Phase 3) with explicit user approval via AskUserQuestion.

## Purpose

Pipeline-safe variant of sync.md. Synchronize documentation with code changes, verify project quality, and finalize pull requests. This workflow is used by the auto pipeline and does not include merge functionality.

## Scope

- Implements Step 4 of AE's 4-step workflow (Report and Commit)
- Receives implementation artifacts from /ae run
- Produces synchronized documentation, commits, and PR readiness
- Does not include merge operations — merge is controlled by the auto pipeline orchestrator
...
- When `.ae/project/` documents are outdated
...
3. **Project Document Updates**:
   - Updates `.ae/project/tech.md` when new dependencies/technologies added
   - Updates `.ae/project/structure.md` when architecture changes
   - Updates `.ae/project/product.md` when new features added
   - Updates `.ae/project/codemaps/` when architecture changes detected (delegates to codemaps workflow)
...
- .ae/config/config.yaml (git strategy, language settings)
- .ae/config/sections/git-strategy.yaml (auto_branch, branch creation policy)
- .ae/config/sections/language.yaml (git_commit_messages setting)
- .ae/specs/ directory listing (SPEC documents for sync)
- .ae/project/ directory listing (project documents for conditional update)
...
- Abort: Stop sync, fix tests first (exit via graceful exit)
...
Purpose: Ensure code has appropriate @MX annotations for AI agent context. Supports all 16 AE-ADK languages.
...
2. Show message: "Run /ae run to add missing tags, or use --skip-mx to bypass"
...
- Create backup directory: .ae/backups/sync-{timestamp}/
- Copy critical files: README.md, docs/, .ae/specs/
...
- Generate sync report: .ae/reports/sync-report-{timestamp}.md
...
Purpose: Update .ae/project/ documents when significant structural changes are detected.
...
Skip condition: If .ae/project/ directory does not exist or contains no files, skip this step entirely.
...
- If architectural changes: Regenerate .ae/project/codemaps/ via codemaps workflow (workflows/codemaps.md) when significant structural changes (new directories, dependency graph changes, or module reorganization) are detected
...
Read `github.git_workflow` from `.ae/config/sections/system.yaml`. This determines how changes are delivered.
...
After successful commit, create a session boundary tag to enable `/ae context` reconstruction:

```
git tag -a "ae/SPEC-{ID}/sync-complete" \
  -m "Sync phase completed
SPEC: SPEC-XXX
Docs updated: N files
Coverage verified: XX%
Context embedded in: [commit hash]
Next action: Feature complete or /ae plan for next SPEC"
```

Tag naming convention: `ae/SPEC-{ID}/sync-complete`

**Context Memory Integration:**

The embedded context enables:

1. **Session Resumption**: When resuming development, `/ae context` retrieves this information automatically
...
- Context metadata saved to `.ae/state/sync-context-{SPEC-ID}.json` for quick access
...
GOOS=linux   GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o /tmp/ci-build-linux-amd64     ./cmd/ae/ &
GOOS=linux   GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o /tmp/ci-build-linux-arm64     ./cmd/ae/ &
GOOS=darwin  GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o /tmp/ci-build-darwin-amd64    ./cmd/ae/ &
GOOS=darwin  GOARCH=arm64 CGO_ENABLED=0 go build -ldflags="-s -w" -o /tmp/ci-build-darwin-arm64    ./cmd/ae/ &
GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-s -w" -o /tmp/ci-build-windows-amd64.exe ./cmd/ae/ &
wait
```
...
- Phase 3: Changes committed, local CI mirror validated (Step 3.1.5: vet + test-race + lint + cross-compile — Windows skipped), delivered per git_workflow strategy (PR created for github_flow/gitflow, direct push for main_direct)
