...
- Triggered by: /ae review --team OR explicit multi-perspective review request
...
2. Create team:
   ```
   TeamCreate(team_name: "ae-review-{target}")
   ```
...
All four reviewers run in parallel. Messages from teammates are delivered automatically to AE.
