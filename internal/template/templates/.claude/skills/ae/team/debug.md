...
- Triggered by: /ae fix --team OR auto-detected when issue has multiple potential root causes
...
3. Create team:
   ```
   TeamCreate(team_name: "ae-debug-{issue-slug}")
   ```
...
Teammate 1 - hypothesis-1 (general-purpose agent, researcher role, haiku model):
...
Teammate 2 - hypothesis-2 (general-purpose agent, researcher role, haiku model):
...
Teammate 3 - hypothesis-3 (general-purpose agent, researcher role, haiku model):
