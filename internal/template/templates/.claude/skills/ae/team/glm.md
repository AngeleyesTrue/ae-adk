---
name: ae-workflow-team
description: >
  CG (Claude + GLM) hybrid mode for AE-ADK. Uses tmux pane-level environment
...
# AE CG Mode (Claude + GLM via tmux Agent Teams)
...
```
User runs: ae glm sk-xxx     (save API key, once)
User runs: tmux new -s ae    (start tmux session)
User runs: ae cg             (configure CG mode)
    │
...
Read `.ae/config/sections/llm.yaml` for `team_mode` value:
...
Detection steps:
1. Read `.ae/config/sections/llm.yaml`
...
`ae cg` injects these into the tmux session:
...
|-------|------|-----|-------|
| Leader (AE) | Original | Claude | User's choice (Opus/Sonnet) |
| implementer | New pane | Z.AI | glm-5 / glm-4.7 |
| tester | New pane | Z.AI | glm-5 / glm-4.7 |
| designer | New pane | Z.AI | glm-5 / glm-4.7 |
| researcher | New pane | Z.AI | glm-4.7-flashx |
| reviewer | New pane | Z.AI | glm-4.7-flashx |
...
| No API key | Error: "Run ae glm <api-key> first" |
