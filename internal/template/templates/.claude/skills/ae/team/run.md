...
From `.ae/config/sections/workflow.yaml` → `team.role_profiles`:
...
This workflow is loaded ONLY when team mode has been explicitly selected (via `--team` flag or auto-selection). Check `.ae/config/sections/llm.yaml` to determine WHICH team mode to use:
...
the tmux session has GLM env vars injected by `ae cg`.

### Env Isolation Mechanism (Verified)

`ae cg` executes two complementary steps:
...
- `ae cg` has been run inside tmux (team_mode="cg" in llm.yaml)
- Claude Code started in the SAME pane where `ae cg` was run
- GLM API key saved via `ae glm setup <key>`
...
1. **Delegate to manager-spec subagent**:
   ...
   Agent(
     ...
     prompt: "Create SPEC document for: {user_description}
              Follow EARS format.
              Output to: .ae/specs/SPEC-XXX/spec.md"
   )
   ```

2. **User Approval** via AskUserQuestion

3. **Output**: `.ae/specs/SPEC-XXX/spec.md`
...
1. Create team:
   ```
   TeamCreate(team_name: "ae-run-SPEC-XXX")
   ```
...
- See `.claude/rules/ae/workflow/worktree-integration.md` Prompt Path Rules section
...
```
Agent(subagent_type: "general-purpose", team_name: "ae-run-SPEC-XXX", name: "backend-dev", model: "sonnet", mode: "acceptEdits", isolation: "worktree", prompt: "Backend role. File ownership: server-side code. ...")
Agent(subagent_type: "general-purpose", team_name: "ae-run-SPEC-XXX", name: "frontend-dev", model: "sonnet", mode: "acceptEdits", isolation: "worktree", prompt: "Frontend role. File ownership: client-side code. ...")
Agent(subagent_type: "general-purpose", team_name: "ae-run-SPEC-XXX", name: "tester", model: "sonnet", mode: "acceptEdits", isolation: "worktree", prompt: "Testing role. File ownership: test files exclusively. ...")
```
