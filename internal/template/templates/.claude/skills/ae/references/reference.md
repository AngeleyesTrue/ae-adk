...
description: >
  ...
  and resume patterns used across all AE workflows. Provides context
...
Common patterns, flag reference, and legacy command mapping used across all AE workflows.

For configuration file paths, see: CLAUDE.md Section 9
For error handling delegation, see: CLAUDE.md Section 11
For completion markers, see: .claude/rules/ae/core/ae-constitution.md
For development mode details, see: .claude/rules/ae/workflow/workflow-modes.md
...
- AE Workflow: Parallel exploration phase, then sequential SPEC generation and DDD/TDD implementation
...
- Read existing SPEC document from .ae/specs/SPEC-XXX/
...
- ae --resume SPEC-XXX: Resume full autonomous workflow from last phase
...
Previous /ae:X-Y command format mapped to new /ae subcommand format:

- /ae:0-project maps to /ae project
- /ae:1-plan maps to /ae plan
- /ae:2-run maps to /ae run
- /ae:3-sync maps to /ae sync
- /ae:9-feedback maps to /ae feedback
- /ae:fix maps to /ae fix
- /ae:loop maps to /ae loop
- /ae:ae maps to /ae (default autonomous workflow)
- /ae:review maps to /ae review
- /ae:clean maps to /ae clean
- /ae:codemaps maps to /ae codemaps
- /ae:coverage maps to /ae coverage
- /ae:e2e maps to /ae e2e
- /ae:mx maps to /ae mx

Note: /ae:99-release is a separate local-only command, not part of the /ae skill.
