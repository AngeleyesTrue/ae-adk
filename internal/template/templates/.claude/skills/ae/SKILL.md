...
name: ae
description: >
  AE super agent - unified orchestrator for autonomous development.
...
- Quality gates, security boundaries: .claude/rules/ae/core/ae-constitution.md
- SPEC workflow phases, token budgets: .claude/rules/ae/workflow/spec-workflow.md
- Development methodologies (DDD/TDD): .claude/rules/ae/workflow/workflow-modes.md
- Agent definitions: See CLAUDE.md Section 4. For agent creation, use builder-agent subagent.
- @MX tag rules and protocol: .claude/rules/ae/workflow/mx-tag-protocol.md
...
- **codemaps**: Generate architecture documentation in `.ae/project/codemaps/`
...
- Implementation language (implement, build, create, add, develop) with clear scope routes to **ae** (default autonomous)
...
If the intent is clearly a development task with no specific routing signal, default to the **ae** workflow (plan -> run -> sync pipeline) for full autonomous execution.
...
### (default) - AE Autonomous Workflow
...
For detailed orchestration: Read ${CLAUDE_SKILL_DIR}/workflows/ae.md
...
Output: product.md, structure.md, tech.md in .ae/project/
...
Before executing plan, run, sync, fix, loop, or default workflows, verify project documentation exists by checking for `.ae/project/product.md`. If product.md does NOT exist, use AskUserQuestion to ask the user (in their conversation_language):

Question: Project documentation not found. Would you like to create it first?
Options:
- Create project documentation (Recommended): Generates product.md, structure.md, tech.md through a guided interview. This helps AE understand your project context for better results in all subsequent workflows.
- Skip and continue: Proceed without project documentation. AE will have less context about your project.

This check does NOT apply to: project, feedback subcommands.

[HARD] Beginner-Friendly Option Design:
All AskUserQuestion calls throughout AE workflows MUST follow these rules:
...
Load relevant configuration from .ae/config/config.yaml and section files as needed.
...
When all workflow phases complete successfully, add the appropriate completion marker (`<ae>DONE</ae>` or `<ae>COMPLETE</ae>`).
