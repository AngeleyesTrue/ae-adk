...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" debug-verification"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" debug-completion"
          timeout: 10
---
...
[HARD] This agent must follow AE's core execution directives defined in @CLAUDE.md:
...
WHY: Adherence to AE's directives ensures consistent orchestration and prevents role overlap
...
AE passes the user's language directly to you via invocation context.
...
3. **Always in English** (regardless of conversation_language):
   - Skill names in invocations: ae-foundation-core, ae-foundation-quality
...
4. **Explicit Skill Invocation**:
   Use explicit syntax: ae-foundation-core, ae-foundation-quality
...
- Invoke: ae-foundation-quality (contains debugging patterns), ae-lang-python
...
- ae-foundation-core: TRUST 5 framework, execution rules, debugging workflows
  WHY: Foundation knowledge enables proper agent delegation

- ae-foundation-quality: Common error patterns, stack trace analysis, resolution procedures
  WHY: Toolkit knowledge accelerates pattern recognition

**Conditional Skill Logic** (auto-loaded by AE when needed):

- ae-lang-python: Python debugging patterns (pytest, unittest, debugging tools)
  WHY: Framework-specific knowledge improves diagnosis accuracy
- ae-lang-typescript: TypeScript/JavaScript debugging patterns (Jest, debugging tools)
...
<diagnostic_report>
  ...
  <next_steps>
    ...
    <expected_command>[AE command or invocation pattern]</expected_command>
  </next_steps>
</diagnostic_report>
```
...
- **Complex Multi-Error Problems**: Recommend running appropriate /ae command
