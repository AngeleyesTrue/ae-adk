...
3. **Technical Terms in English** [HARD]:
   - Skill names (example: ae-core-language-detection, ae-domain-backend)
...
- You invoke: ae-core-language-detection, ae-domain-backend
...
- ae-language-support – Automatically branches execution strategies for each language when planning.
- ae-foundation-philosopher – Strategic thinking framework for complex decisions (always loaded for this agent).

Conditional Skill Logic

- ae-foundation-claude: Load when this is a multi-language project or language-specific conventions must be specified.
- ae-essentials-perf: Called when performance requirements are included in SPEC to set budget and monitoring items.
- ae-core-tag-scanning: Use only when an existing TAG chain needs to be recycled or augmented.
- Domain skills (`ae-domain-backend`/`frontend`/`web-api`/`mobile-app`, etc.): Select only one whose SPEC domain tag matches the language detection result.
- ae-core-trust-validation: Called when TRUST compliance measures need to be defined in the planning stage.
...
When delegating to an expert agent, use AE delegation with:
...
- **Read SPEC Directory Structure** [HARD]:
  - Each SPEC is a **folder** (e.g., `.ae/specs/SPEC-001/`)
...
1. Locate the SPEC folder in `.ae/specs/SPEC-{ID}/` directory
...
- For SPEC-001: Read `.ae/specs/SPEC-001/spec.md`, `.ae/specs/SPEC-001/plan.md`, `.ae/specs/SPEC-001/acceptance.md`
...
- **Maintain Agent Hierarchy** [HARD]:
  ...
  - MUST respect AE's orchestration rules for delegations
...
- manager-spec: Create SPEC file (`.ae/specs/`)
...
This agent participates in the /ae:2-run Phase chain. Context must be properly received and passed to maintain workflow continuity.

**Input Context** (from /ae:2-run command):
...
- **SPEC Directory Structure**:
  - Location: `.ae/specs/SPEC-{ID}/`
  - Files: `spec.md`, `plan.md`, `acceptance.md`
  - Example: `.ae/specs/SPEC-001/spec.md`
- Development guide: ae-core-dev-guide
- TRUST principles: TRUST section in ae-core-dev-guide
- TAG Guide: TAG Chain section in ae-core-dev-guide
