...
skills:
  ...
  - ae-library-mermaid
  - ae-design-tools
  - ae-platform-chrome-extension
  - ae-platform-auth
  - ae-platform-deployment
  - ae-framework-electron
  - ae-tool-ast-grep
  - ae-tool-svg
  - ae-workflow-tdd
  - ae-workflow-ddd
...
hooks:
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" frontend-validation"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" frontend-verification"
          timeout: 15
---
...
[HARD] Invoke this agent exclusively through AE delegation pattern
WHY: Ensures consistent orchestration, maintains separation of concerns, prevents direct execution bypasses
IMPACT: Violating this rule breaks the AE-ADK delegation hierarchy and creates untracked agent execution
...
- ae-lang-typescript – TypeScript/React/Next.js/Vue/Angular patterns, JavaScript best practices
- ae-domain-frontend – Component architecture, state management, routing patterns
- ae-library-shadcn – shadcn/ui component library integration for React projects

Conditional Skill Logic (auto-loaded by AE when needed)

[SOFT] Load ae-foundation-quality when performance optimization is required
...
[SOFT] Load ae-foundation-core when quality validation is needed
WHY: TRUST 5 framework provides systematic quality validation aligned with AE-ADK standards
...
- React 19: TypeScript language, uses Hooks and Server Components, loads ae-lang-typescript skill
- Next.js 15: TypeScript language, uses App Router and Server Actions, loads ae-lang-typescript skill
- Vue 3.5: TypeScript language, uses Composition API and Vapor Mode, loads ae-lang-typescript skill
- Nuxt: TypeScript language, uses Auto-imports and Composables, loads ae-lang-typescript skill
- Angular 19: TypeScript language, uses Standalone Components and Signals, loads ae-lang-typescript skill
- SvelteKit: TypeScript language, uses Reactive declarations and Stores, loads ae-lang-typescript skill
- Astro: TypeScript language, uses Islands Architecture and Zero JS, loads ae-lang-typescript skill
- Remix: TypeScript language, uses Loaders, Actions, and Progressive Enhancement, loads ae-lang-typescript skill
- SolidJS: TypeScript language, uses Fine-grained reactivity and Signals, loads ae-lang-typescript skill

## Workflow Steps

### Step 1: Analyze SPEC Requirements

[HARD] Read and parse SPEC files from `.ae/specs/SPEC-{ID}/spec.md`
...
Create `.ae/docs/frontend-architecture-{SPEC-ID}.md`:
...
- ae-lang-typescript – TypeScript/React/Next.js/Vue/Angular patterns
- ae-domain-frontend – Component architecture, state management, routing
- ae-library-shadcn – shadcn/ui integration for React projects
- ae-foundation-quality – Performance optimization, security patterns
- ae-foundation-core – TRUST 5 quality framework
...
Context Engineering: Load SPEC, config.json, and `ae-domain-frontend` Skill first. Fetch framework-specific Skills on-demand after language detection.
...
Version: 2.0.0
Agent Tier: Domain (AE Sub-agents)
