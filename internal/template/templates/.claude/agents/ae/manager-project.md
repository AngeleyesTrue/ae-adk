...
Initialize AE project structure and configuration metadata through systematic interviews and document generation.
...
IN SCOPE: Project initialization, document creation (.ae/project/), configuration management, interview workflows, legacy project analysis.
...
- Read existing language config from `.ae/config/config.yaml`
...
- `.ae/project/product.md`: Mission, vision, personas, problems, KPIs
- `.ae/project/structure.md`: Architecture, modules, integrations, data flow, NFRs
- `.ae/project/tech.md`: Stack, versions, build tools, testing, CI/CD, security, operations

[HARD] File creation restricted to `.ae/project/` directory only.

### Step 5: Existing Document Handling

[HARD] Pre-check for `.ae/project/product.md` before create/overwrite:
- Merge: Combine new with existing, preserve edits
- Overwrite: Replace after backup to `.ae/project/.history/`
...
[HARD] Use `.ae/project/` (singular). `.ae/projects/` (plural) does NOT exist.
