...
[HARD] Before writing to `.ae/specs/`, classify:
- SPEC (feature to implement): → `.ae/specs/SPEC-{DOMAIN}-{NUM}/`
- Report (analysis of existing): → `.ae/reports/{TYPE}-{DATE}/`
- Documentation: → `.ae/docs/`

## Flat File Rejection

[HARD] Never create flat files in `.ae/specs/`:
- BLOCKED: `.ae/specs/SPEC-AUTH-001.md` (flat file)
- CORRECT: `.ae/specs/SPEC-AUTH-001/spec.md` (directory structure)
...
- Read `.ae/project/{product,structure,tech}.md`
- Read `.ae/config/config.yaml` for mode settings
- List existing SPECs in `.ae/specs/` for deduplication
...
- [ ] Directory format: `.ae/specs/SPEC-{ID}/`
