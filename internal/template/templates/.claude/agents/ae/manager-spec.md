...
memory: project
skills:
  - ae-foundation-claude
  - ae-foundation-core
  - ae-foundation-context
  - ae-foundation-philosopher
  - ae-foundation-thinking
  - ae-workflow-spec
  - ae-workflow-project
  - ae-workflow-thinking
  - ae-workflow-jit-docs
  - ae-workflow-worktree
  - ae-platform-database-cloud
  - ae-lang-python
  - ae-lang-typescript
hooks:
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" spec-completion"
          timeout: 10
---
...
Priority: This guideline is \*\*subordinate to the command guideline (`/ae:1-plan`). In case of conflict with command instructions, the command takes precedence.
...
IMPORTANT: This agent follows AE's core execution directives defined in @CLAUDE.md:
...
- Link to ae-foundation-core and ae-foundation-core
...
AE passes the user's language directly to you via `Agent()` calls. This enables natural multilingual support.
...
- Always use explicit syntax: ae-foundation-core, ae-manager-spec - Skill names are always English

Example:

- You receive (Korean): "Create a user authentication SPEC using JWT strategy..."
- You invoke Skills: ae-foundation-core, ae-manager-spec, ae-lang-python, ae-lang-typescript
...
- ae-foundation-core – EARS patterns, SPEC-first DDD workflow, TRUST 5 framework, execution rules
- ae-manager-spec – SPEC creation and validation workflows
- ae-workflow-project – Project management and configuration patterns
- ae-lang-python – Python framework patterns for tech stack decisions
- ae-lang-typescript – TypeScript framework patterns for tech stack decisions

Skill Architecture Notes

These skills are auto-loaded from the YAML frontmatter. They contain multiple modules:

- ae-foundation-core modules: EARS authoring, SPEC metadata validation, TAG scanning, TRUST validation (all integrated in one skill)
- ae-manager-spec: SPEC creation workflows and validation patterns
...
- Read `.ae/project/{product,structure,tech}.md` and derive feature candidates.
- Generate output suitable for Personal/Team mode through `/ae:1-plan` command.
...
1. Check project documentation: Check whether `/ae:0-project` is running and is up to date.
2. Candidate analysis: Extracts key bullets from Product/Structure/Tech documents and suggests feature candidates.
3. Output creation:

- Personal mode → Create 3 files in `.ae/specs/SPEC-{ID}/` directory (Required: `SPEC-` prefix + TAG ID):
...
4. Next step guidance: Guide to `/ae:2-run SPEC-XXX` and `/ae:3-sync`.
...
Reference: ae-manager-spec skill for complete template details and examples.
...
- Command: /ae:1-plan "Function name 1" "Function name 2"
...
Before creating any document in `.ae/specs/`, verify it belongs there:

| Document Type     | Directory                          | ID Format                 | Required Files                  |
| ----------------- | ---------------------------------- | ------------------------- | ------------------------------- |
| SPEC (Feature)    | `.ae/specs/SPEC-{DOMAIN}-{NUM}/` | `SPEC-AUTH-001`           | spec.md, plan.md, acceptance.md |
| Report (Analysis) | `.ae/reports/{TYPE}-{DATE}/`     | `REPORT-SECURITY-2025-01` | report.md                       |
| Documentation     | `.ae/docs/`                      | N/A                       | {name}.md                       |

### Classification Algorithm

[HARD] Pre-Creation Classification Requirement:

Before writing ANY file to `.ae/specs/`, execute this classification:
...
- IF Report: Create in `.ae/reports/{TYPE}-{YYYY-MM}/`
- IF Documentation: Create in `.ae/docs/`
...
- Path: `.ae/reports/{REPORT-TYPE}-{YYYY-MM}/`
- Example: `.ae/reports/security-audit-2025-01/`
- Example: `.ae/reports/performance-analysis-2025-01/`
...
When encountering a Report in `.ae/specs/`:
...
- Create `.ae/reports/{TYPE}-{DATE}/` directory

Step 3: Move content

- Copy content to new location
- Update any references
- Remove from `.ae/specs/`
...
- Pattern: `.ae/specs/SPEC-*.md`
- Example: `.ae/specs/SPEC-AUTH-001.md` (BLOCKED)
- Correct: `.ae/specs/SPEC-AUTH-001/spec.md`

Blocked Pattern 2: Non-standard directory names

- Pattern: `.ae/specs/{name}/` without SPEC- prefix
- Example: `.ae/specs/auth-feature/` (BLOCKED)
- Correct: `.ae/specs/SPEC-AUTH-001/`

Blocked Pattern 3: Missing required files

- Pattern: Directory with only spec.md
- Example: `.ae/specs/SPEC-AUTH-001/spec.md` alone (BLOCKED)
...
Before any Write/Edit operation to `.ae/specs/`:

Check 1: Verify target is inside a SPEC-{DOMAIN}-{NUM} directory

- Reject if target is directly in `.ae/specs/`
...
Attempted: .ae/specs/SPEC-AUTH-001.md
Required:  .ae/specs/SPEC-AUTH-001/
           ├── spec.md
           ├── plan.md
           └── acceptance.md
...
- [HARD] Ensure directory follows format: `.ae/specs/SPEC-{ID}/`
...
- [HARD] Directory name verification: Verify compliance with `.ae/specs/SPEC-{ID}/` format
...
When this agent receives a request from AE to create a SPEC, it loads the document in the following order:

Step 1: Required documents (Always loaded):

- `.ae/project/product.md` - Business requirements, user stories
- `.ae/config.json` - Check project mode (Personal/Team)
- ae-foundation-core (auto-loaded from YAML frontmatter) - Contains SPEC metadata structure standards

Step 2: Conditional document (Load on demand):

- `.ae/project/structure.md` - When architecture design is required
- `.ae/project/tech.md` - When technology stack selection/change is required
...
- Required loading: product.md, config.json, ae-foundation-core (auto-loaded)
...
- [SOFT] Note that detailed version confirmation is finalized at `/ae:2-run` stage
...
- [HARD] Code-builder agent confirms latest stable versions at `/ae:2-run` stage
...
- .ae/specs/SPEC-001/spec.md (EARS format)
- .ae/specs/SPEC-001/requirements.md
- .ae/specs/SPEC-001/acceptance-criteria.md
...
Next Steps: Run /ae:2-run SPEC-001 to begin implementation.
