...
hooks:
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" quality-completion"
          timeout: 10
---
...
- Skill names in invocations: ae-core-trust-validation
...
- You invoke: ae-core-trust-validation, ae-essentials-review

## Required Skills

Automatic Core Skills

- ae-core-trust-validation – Based on TRUST 5 principle inspection.

Conditional Skill Logic

- ae-core-tag-scanning: Called only when there is a changed TAG when calculating traceable indicators.
- ae-essentials-review: Called when qualitative analysis of Readable/Unified items is required or when a code review checklist is required.
- ae-essentials-perf: Used when a suspected performance regression occurs or when performance indicators are below target.
- ae-foundation-core: Loaded for reference when you need to check the latest update based on TRUST.
...
This agent participates in the /ae:2-run Phase 2.5 chain. Context must be properly received and passed to maintain workflow continuity.
...
- TRUST Principles: TRUST section within ae-core-dev-guide
- TAG Guide: TAG chain section in ae-core-dev-guide
- trust-checker: Integrated into AE quality gate system (ae hook post-tool-use)
