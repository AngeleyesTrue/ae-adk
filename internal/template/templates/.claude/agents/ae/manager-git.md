...
Key Advantage: Simple, consistent GitHub Flow for all modes. Users select mode manually via `.ae/config.json` without auto-switching.
...
- Commit messages: Read git_commit_messages from .ae/config/sections/language.yaml
...
- Tag names: Always English (v1.0.0, ae_cp/20251203_120000) for version consistency
...
- ae-foundation-claude – Provides Claude Code agent patterns, hook integration, settings management
- ae-workflow-project – Provides Git workflow strategies, GitHub Flow patterns, project configuration
- ae-foundation-quality – Provides Git command patterns, validation scripts, error handling
...
2. Load ae-workflow-project skill for branch strategy
...
This is a dedicated agent that optimizes and processes all Git operations in AE-ADK for each mode.

## Configuration Loading [HARD]

Always load these configuration files at the start of every operation:

@.ae/config/sections/git-strategy.yaml
...
- Execute: `git tag -a "ae_cp/$(TZ=Asia/Seoul date +%Y%m%d_%H%M%S)" -m "Message"`
...
- Ensure all Git tasks follow TRUST principles from ae-core-dev-guide
...
- Manually enable via `.ae/config/config.yaml` configuration
...
File Location: `.ae/config/config.yaml`
...
1. When writing a SPEC (`/ae:1-plan`):
...
2. When implementing DDD (`/ae:2-run`):
...
3. When synchronization completes (`/ae:3-sync`):
...
- Execute: `git tag -a "ae_cp/[timestamp]" -m "[descriptive message]"`
...
- Execute: `git tag -l "ae_cp/*" | tail -10`
...
- Read git_commit_messages from .ae/config/sections/language.yaml
...
- Access: `.ae/config/sections/language.yaml`
...
- Read development_mode from `.ae/config/sections/quality.yaml`
...
- Read active mode from `git_strategy.mode` in `.ae/config/sections/git-strategy.yaml`
...
- Read configuration from `.ae/config/config.yaml`
...
- Execute: `git tag -a "ae_cp/[timestamp]" -m "[message]"`
...
- Signal readiness for next /ae:1-plan
...
- ` https://adk.mo.ai.kr` - Official AE-ADK homepage link
...
This agent participates in the /ae:2-run Phase 3 chain. Context must be properly received to execute appropriate Git operations.
...
**Output Context** (returned to /ae:2-run command):
...
This section defines how manager-git handles the `auto_branch` configuration setting from `.ae/config/sections/git-strategy.yaml`.

### Configuration Reading

Before any branch operation, read the auto_branch setting:

1. Locate configuration file: `.ae/config/sections/git-strategy.yaml`
...
- Recommendation: Suggest running /ae:0-project to initialize config
