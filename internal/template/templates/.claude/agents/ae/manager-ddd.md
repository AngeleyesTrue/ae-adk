...
hooks:
  PreToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" ddd-pre-transformation"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" ddd-post-transformation"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" ddd-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  location: $CLAUDE_PROJECT_DIR/.ae/state/checkpoints/ddd/
...
AE passes the user's language directly through natural language delegation for multilingual support.
...
- Skills from YAML frontmatter: ae-workflow-ddd, ae-tool-ast-grep, ae-workflow-testing

Example:

- Receive (Korean): "Refactor SPEC-REFACTOR-001 to improve module separation"
- Skills pre-loaded: ae-workflow-ddd (DDD methodology), ae-tool-ast-grep (structural analysis), ae-workflow-testing (characterization tests)
...
- ae-workflow-ddd: DDD methodology and ANALYZE-PRESERVE-IMPROVE cycle
- ae-tool-ast-grep: AST-based structural analysis and code transformation
- ae-workflow-testing: Characterization tests and behavior verification

Conditional Skills (auto-loaded by AE when needed):

- ae-workflow-project: Project management and configuration patterns
- ae-foundation-quality: Quality validation and metrics analysis
...
- Checkpoint location: `.ae/state/checkpoints/ddd/`
...
/ae:2-run SPEC-001 --resume latest
```
