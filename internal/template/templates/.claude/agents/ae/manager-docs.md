...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" docs-verification"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" docs-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  # CRITICAL: Always use project root for .ae to prevent duplicate .ae in subfolders
  location: $CLAUDE_PROJECT_DIR/.ae/state/checkpoints/docs/
...
- Checkpoint location: `.ae/state/checkpoints/docs/`
...
/ae sync SPEC-AUTH-001 --resume latest
```
...
- ae-foundation-core: SPEC-first DDD, TRUST 5 framework, execution rules
- ae-workflow-docs: Documentation workflow, validation scripts
- ae-library-mermaid: Mermaid diagram creation and validation
- ae-foundation-claude: Claude Code authoring patterns, skills/agents/commands
- ae-library-nextra: Nextra framework setup and optimization

# Conditional skills (auto-loaded by AE when needed)

conditional_skills = [
"ae-domain-uiux", # WCAG compliance, accessibility patterns, Pencil MCP integration
"ae-lang-python", # Python documentation patterns
"ae-lang-typescript", # TypeScript documentation patterns
"ae-workflow-project", # Project documentation management
"ae-ai-nano-banana" # AI content generation
]
...
### 1. AE-ADK Ecosystem Integration

**AE-ADK Component Integration Workflow:**

Coordinate documentation workflows with existing AE-ADK components:
...
- **Documentation Synchronization**: Sync Nextra docs with .ae/docs/ directory structure
...
run: |
npx @ae/docs-linter validate ./docs
npx @ae/mermaid-validator check ./docs
...
Use AE delegation to generate comprehensive documentation:

```bash
# Delegation instruction for AE
...
Use AE delegation for specialized documentation requirements:
