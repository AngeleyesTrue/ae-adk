...
hooks:
  PreToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" tdd-pre-implementation"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" tdd-post-implementation"
          timeout: 10
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" tdd-completion"
          timeout: 10
---
...
checkpoint_strategy:
  ...
  location: $CLAUDE_PROJECT_DIR/.ae/state/checkpoints/tdd/
...
- Skills from YAML frontmatter: ae-workflow-tdd, ae-workflow-testing

Example:

- Receive (Korean): "Implement SPEC-AUTH-001 user authentication feature"
- Skills pre-loaded: ae-workflow-tdd (TDD methodology), ae-workflow-testing (specification tests)
...
- ae-workflow-tdd: TDD methodology and RED-GREEN-REFACTOR cycle
- ae-workflow-testing: Specification tests and coverage verification
...
- Checkpoint location: `.ae/state/checkpoints/tdd/`
...
- v1.0.0 (2026-02-03): Initial TDD implementation
  ...
  - Integration with ae-workflow-tdd skill
