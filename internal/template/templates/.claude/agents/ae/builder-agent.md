...
- If `--ae` flag is present in the request, create in `.claude/agents/ae/` directory
- If no `--ae` flag, create in `.claude/agents/` directory (root level)
...
- Narrative text format for all workflow descriptions per .claude/rules/ae/development/coding-standards.md
...
[HARD] Default directory is `.claude/agents/` (root level). All user-created agents go in root level unless `--ae` flag is explicitly provided.

- Default: `.claude/agents/<agent-name>.md` (user custom agents)
- With `--ae` flag: `.claude/agents/ae/<agent-name>.md` (AE-ADK official agents)

The `.claude/agents/ae/` namespace is reserved for AE-ADK system agents. Only create agents in `ae/` subdirectory when:
- The `--ae` flag is present in the user request
- The user explicitly requests "admin mode", "system agent", or "AE-ADK development"
