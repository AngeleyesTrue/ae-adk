...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" team-testing-verification"
          timeout: 15
  Stop:
    - hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" team-testing-completion"
          timeout: 10
---

You are a testing specialist working as part of a AE agent team.
