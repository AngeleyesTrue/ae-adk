...
- YAML frontmatter configuration with official and AE-extended fields
...
- If `--ae` flag is present in the request, use `ae-` prefix instead of `my-`
...
Create SKILL.md and supporting files in `.claude/skills/<prefix>-<name>/` directory (prefix is `my-` by default, or `ae-` with `--ae` flag). Apply frontmatter, write content sections, and verify line count.
...
### AE-ADK Extended Fields

These fields are NOT in the official Claude Code spec but are used by AE-ADK system skills:
...
[HARD] Default prefix is `my-`. All user-created skills use `my-` prefix unless `--ae` flag is explicitly provided.

- Default: `my-<name>` → directory `.claude/skills/my-<name>/`
- With `--ae` flag: `ae-<name>` → directory `.claude/skills/ae-<name>/`

The `ae-` namespace is reserved for AE-ADK system skills. Only use `ae-` prefix when:
- The `--ae` flag is present in the user request
- The user explicitly requests "admin mode", "system skill", or "AE-ADK development"
...
[HARD] Skills MUST be created in `.claude/skills/` directory, NEVER in `.ae/skills/`.

[HARD] The skill file MUST always be named `SKILL.md`. NEVER name it after the skill or use any other filename.

[HARD] The full skill name (e.g., `ae-library-pykrx`) is used as a SINGLE directory name. NEVER create nested subdirectories by splitting the name on hyphens. Hyphens are part of the directory name, not path separators.
...
Examples:
- `.claude/skills/ae-library-pykrx/SKILL.md`  ✅ correct
- `.claude/skills/my-analyzing-code/SKILL.md` ✅ correct
- `.claude/skills/ae/library/pykrx.md`          ❌ WRONG: nested dirs + wrong filename
- `.claude/skills/ae-library/pykrx.md`           ❌ WRONG: wrong filename
...
Tool Permissions by AE Category:
