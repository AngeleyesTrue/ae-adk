...
- Default prefix: `my-`. With `--ae` flag: `ae-` prefix
...
- Plan YAML frontmatter with required fields (name, description) and AE extensions
...
- CORRECT: `.claude/skills/{skill-name}/SKILL.md` (e.g., `.claude/skills/ae-library-pykrx/SKILL.md`)
- WRONG: `.claude/skills/ae/library/pykrx.md` (nested subdirectories)
...
- Naming prefixes: `ae-{category}-{name}` (system), `my-{name}` or `custom-{name}` (user)
