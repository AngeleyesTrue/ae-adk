...
```json
{
  "hooks": {
    "SessionStart": [{
      "type": "command",
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-session-start.sh\"",
      "timeout": 5
    }],
    "PreCompact": [{
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-compact.sh\"",
      "timeout": 5
    }],
    "PreToolUse": [{
      "matcher": "Write|Edit|Bash",
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-pre-tool.sh\"",
      "timeout": 5
    }],
    "PostToolUse": [{
      "matcher": "Write|Edit",
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-post-tool.sh\"",
      "timeout": 60
    }],
    "Stop": [{
      "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-stop.sh\"",
      "timeout": 5
    }],
    "TeammateIdle": [{
      "hooks": [{
        "type": "command",
        "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\"",
        "timeout": 10
      }]
    }],
    "TaskCompleted": [{
...
```json
{
  "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/hook.sh\""
}
```

**Important**: Quote the entire path to handle project folders with spaces:
- Correct: `"\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/hook.sh\""`
- Wrong: `"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/hook.sh"`

For StatusLine path configuration, see @settings-management.md (StatusLine does NOT support environment variables).

## Hook Wrappers

AE-ADK generates hook wrapper scripts during `ae init` that:

1. Read stdin JSON from Claude Code
2. Forward it to the ae binary via `ae hook <event>` command
3. Support multiple ae binary locations:
   - `ae` command in PATH
   - Detected Go bin path from initialization
   - Default `~/go/bin/ae`

Wrapper scripts are located at:
- `.claude/hooks/ae/handle-session-start.sh`
- `.claude/hooks/ae/handle-compact.sh`
- `.claude/hooks/ae/handle-pre-tool.sh`
- `.claude/hooks/ae/handle-post-tool.sh`
- `.claude/hooks/ae/handle-stop.sh`
- `.claude/hooks/ae/handle-agent-hook.sh`: TeammateIdle, TaskCompleted events (team mode)
...
2. Check if file has `.ae/config/sections/mx.yaml` exclusion
...
if [[ "$TOOL_NAME" =~ ^(Write|Edit)$ ]] && is_source_file "$FILE_PATH"; then
  ...
  ae mx check --file "$FILE_PATH" --dry
fi
```
...
- Use `/ae mx --dry` to preview tag recommendations
