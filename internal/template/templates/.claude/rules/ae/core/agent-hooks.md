...
hooks:
  PreToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
        - type: command
          command: "\"$CLAUDE_PROJECT_DIR/.claude/hooks/ae/handle-agent-hook.sh\" {action}"
          timeout: 5
  PostToolUse:
    - matcher: "Write|Edit|MultiEdit"
      hooks:
...
| implementer | - | implementer-verification | - |
| tester | - | tester-verification | tester-completion |
| reviewer | - | - | reviewer-completion |

## Hook Command Interface

Agent hooks are executed via `ae hook agent <action>`:

```bash
ae hook agent ddd-pre-transformation
ae hook agent backend-validation
```
