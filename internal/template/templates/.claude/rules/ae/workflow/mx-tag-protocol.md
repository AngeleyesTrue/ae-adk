...
This rule applies to all agents working with source code in the supported programming languages. For full @MX protocol details, see .claude/skills/ae/references/mx-tag.md.
...
Files matching patterns in `.ae/config/sections/mx.yaml` exclude list are not tagged:
...
Per-file limits from `.ae/config/sections/mx.yaml` (defaults):
- `anchor_per_file`: 3
- `warn_per_file`: 5
...
Project-level settings in `.ae/config/sections/mx.yaml`:
...
**IMPORTANT**: @MX tag descriptions MUST respect the `code_comments` setting from `.ae/config/sections/language.yaml`.
...
Before adding @MX tags, agents MUST read `.ae/config/sections/language.yaml` and use the `code_comments` value to determine the tag language.
