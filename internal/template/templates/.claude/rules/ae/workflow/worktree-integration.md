...
Integration guide for AE Worktree and Claude Code Native Worktree systems.

## Overview

AE-ADK supports two complementary worktree systems for isolated development:
...
**AE Worktree** (`~/.ae/worktrees/{ProjectName}/`):
- Persistent, SPEC-scoped workspaces in global home directory
- Managed via `ae worktree` CLI commands
- Used for multi-session SPEC development and team collaboration

## Comparison Table

| Feature | Claude Native | AE |
|---------|--------------|------|
| **Path** | `.claude/worktrees/<name>/` | `~/.ae/worktrees/{Project}/{SPEC}/` |
...
| **CLI** | `claude -w` (user) or `isolation: worktree` (agent) | `ae worktree new/list/remove` |
| **Cleanup** | Automatic on session end | Manual via `ae worktree remove` |
...
Is this a multi-session SPEC development?
  YES → Use AE Worktree (ae worktree new SPEC-XXX)
  NO ↓
...
### Use AE Worktree (`ae worktree`) for:
...
RUN PHASE
  AE Worktree: SPEC implementation, persistent state
...
SYNC PHASE
  AE Worktree: PR creation from persistent workspace
```
...
AE-ADK implements hook handlers for worktree lifecycle events:

| Hook Event | Triggered When | AE Handler |
|-----------|---------------|--------------|
| WorktreeCreate | Agent with isolation: worktree spawns | `ae hook worktree-create` |
| WorktreeRemove | Agent with isolation: worktree terminates | `ae hook worktree-remove` |

Hook scripts are located at:
- `.claude/hooks/ae/handle-worktree-create.sh`
- `.claude/hooks/ae/handle-worktree-remove.sh`
...
| SPEC documents | `.ae/specs/SPEC-XXX/spec.md` | Relative preferred | SPEC files are copied to worktree during checkout |
...
|-------|-------|----------|
| Worktree not found | Removed manually | Run `ae worktree list` to verify |
...
| Hooks not firing | Missing wrapper script | Check `.claude/hooks/ae/` directory |
...
| Run | AE | `~/.ae/worktrees/{Project}/{SPEC}/` |
| Sync | AE | Same as Run phase |
