---
paths: "**/.ae/specs/**,**/.ae/config/sections/quality.yaml"
---

# SPEC Workflow

AE's three-phase development workflow with token budget management.

## Phase Overview

| Phase | Command | Agent | Token Budget | Purpose |
|-------|---------|-------|--------------|---------|
| Plan | /ae plan | manager-spec | 30K | Create SPEC document |
| Run | /ae run | manager-ddd/tdd (per quality.yaml) | 180K | DDD/TDD implementation |
| Sync | /ae sync | manager-docs | 40K | Documentation sync |
...
Output:
- Research document at `.ae/specs/SPEC-XXX/research.md` (deep codebase analysis)
- SPEC document at `.ae/specs/SPEC-XXX/spec.md`
...
- Agent returns structured stagnation report to AE (agents cannot call AskUserQuestion)
- AE presents gap analysis to user via AskUserQuestion with options:
...
Detection method:
- Append acceptance criteria completion count and error count delta to `.ae/specs/SPEC-{ID}/progress.md` at the end of each iteration
...
- `<ae>DONE</ae>` - Task complete
- `<ae>COMPLETE</ae>` - Full completion

## Context Management

/clear Strategy:
- After /ae plan completion (mandatory)
...
- Action: Execute /clear, then /ae run SPEC-XXX

Run to Sync:
- Trigger: Implementation complete, tests passing
- Action: Execute /ae sync SPEC-XXX
...
|-------|---------------|-----------|-----------|
| Plan | manager-spec (single) | researcher + analyst + architect (general-purpose, parallel) | Complexity >= threshold |
| Run | manager-ddd/tdd (sequential) | implementer (backend) + implementer (frontend) + tester (general-purpose, parallel) | Domains >= 3 or files >= 10 |
...
- AE runs annotation cycle with user for plan refinement (1-6 iterations)
- AE synthesizes into SPEC document
...
- Plan phase: .claude/skills/ae/team/plan.md
- Run phase: .claude/skills/ae/team/run.md
- Fix phase: .claude/skills/ae/team/debug.md
- Review: .claude/skills/ae/team/review.md
...
- `workflow.team.enabled: true` in `.ae/config/sections/workflow.yaml`
