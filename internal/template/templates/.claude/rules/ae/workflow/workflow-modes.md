...
Development methodology reference for AE-ADK SPEC workflow.

For phase overview, token strategy, and transitions, see @spec-workflow.md

## Methodology Selection

The Run Phase adapts its workflow based on `quality.development_mode` in `.ae/config/sections/quality.yaml`:
...
- After IMPROVE: Skill("simplify") executes automatically (see run.md Phase 2.10). This is mandatory and not a separate step for the agent — it is orchestrated by AE.
...
- TRUST 5 gates passed (see .claude/rules/ae/core/ae-constitution.md)
...
- After REFACTOR: Skill("simplify") executes automatically (see run.md Phase 2.10). This is mandatory and not a separate step for the agent — it is orchestrated by AE.
...
### Auto-Detection (via /ae project or /ae init)
...
- During init: Use `ae init --mode <ddd|tdd>` flag (default: tdd)
- After project setup: Re-run `/ae project` to auto-detect based on codebase analysis
- Manual edit: Edit `quality.development_mode` in `.ae/config/sections/quality.yaml`
- Per session: Set `AE_DEVELOPMENT_MODE` environment variable
