...
Guidelines for creating custom agents in AE-ADK.
...
- AE-ADK system agents: `.claude/agents/ae/<agent-name>.md` (ae subdirectory)
...
- AE agents run as subagents, so this restriction is not currently applicable
...
| delegate | Coordination-only mode, restricts to team management tools | Team lead agents (AE-specific, experimental) |
...
Role profiles are defined in `.ae/config/sections/workflow.yaml` under `team.role_profiles`:
