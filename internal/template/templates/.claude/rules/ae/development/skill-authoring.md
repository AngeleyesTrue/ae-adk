...
Guidelines for creating AE skills following the Agent Skills open standard (agentskills.io).

## YAML Frontmatter Schema

AE skills follow the Agent Skills standard with AE-specific extensions.

### Standard Fields (agentskills.io)

Required fields:
- name: Skill identifier, lowercase with hyphens, max 64 characters (system: ae-{category}-{name}, user: custom-{name})
...
- AE convention: Use CSV format for consistency with existing skills
...
- Conditional: AskUserQuestion (AE only), Task (managers only)
...
- Mark AE extension fields with standardized comments
