...
Model policy is set via `ae init --model-policy <tier>`:
...
- Activation: `ae cg` (requires tmux)
...
AE's --deepthink flag triggers high effort for the current turn. This aligns with the "deepthink" keyword behavior in Claude Code.
