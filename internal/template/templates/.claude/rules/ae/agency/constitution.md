...
Agency agents and skills may be forked from upstream AE components. Fork management follows these rules:
...
When ae-adk updates (via ae update), Agency checks for upstream changes:
...
## 13. AE Skill Copy Mechanism

Agency can leverage AE skills without forking them. The copy mechanism works as follows:

### Direct Reference (Preferred)

When an Agency agent needs an AE skill, reference it directly in the agent's skills list:
...
No fork entry needed. The skill is used as-is from AE's skill catalog.

### Fork and Customize

When Agency needs a modified version of an AE skill:
...
- [HARD] Never modify AE skill files directly (they are managed by ae update)
