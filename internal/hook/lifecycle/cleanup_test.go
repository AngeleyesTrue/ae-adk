...
func TestCleanup_CleanTempFiles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			baseDir := t.TempDir()
			aeDir := filepath.Join(baseDir, ".ae")

			// Setup directories
			tempDir := filepath.Join(aeDir, "temp")
			if err := os.MkdirAll(tempDir, 0755); err != nil {
				t.Fatalf("failed to create temp dir: %v", err)
			}

			for _, dir := range tt.setupDirs {
				dirPath := filepath.Join(aeDir, dir)
			...
			for _, file := range tt.setupFiles {
				filePath := filepath.Join(aeDir, file)
			...
			config := CleanupConfig{
				TempDir:       filepath.Join(aeDir, "temp"),
				CacheDir:      filepath.Join(aeDir, "cache", "temp"),
				LogDir:        filepath.Join(aeDir, "logs"),
				PreserveState: true,
			}
...
func TestCleanup_PreserveStateFiles(t *testing.T) {
	...
	baseDir := t.TempDir()
	aeDir := filepath.Join(baseDir, ".ae")
	stateDir := filepath.Join(aeDir, "state")
	tempDir := filepath.Join(aeDir, "temp")
