...
// coverageData represents the JSON structure of .ae/state/coverage.json.
...
// loadCoverageData reads coverage data from .ae/state/coverage.json.
...
func loadCoverageData(projectDir string) (float64, bool) {
	coverageFile := filepath.Join(projectDir, defs.AEDir, defs.StateSubdir, "coverage.json")
...
func loadCoverageThreshold(projectDir string) float64 {
	configPath := filepath.Join(projectDir, defs.AEDir, defs.SectionsSubdir, defs.QualityYAML)
...
func loadBaselineCounts(projectDir string) (lsphook.SeverityCounts, error) {
	baselineFile := filepath.Join(projectDir, ".ae", "state", lsphook.BaselineFileName)
