...
// taskMetricsRecord is the JSONL record written to .ae/logs/task-metrics.jsonl.
...
// resolveProjectRoot returns the AE project root for task metrics logging.
...
// not already contain a .ae/ directory, preventing accidental creation of .ae/
...
func resolveProjectRoot(input *HookInput) string {
	...
	// Guard: only write to directories that are already AE project roots.
	// This prevents creating .ae/ in non-project directories when CWD is a
	// subdirectory (e.g., internal/cli/) rather than the project root.
	if _, err := os.Stat(filepath.Join(root, ".ae")); err != nil {
		return ""
	}
	return root
}
...
// .ae/logs/task-metrics.jsonl in the AE project root.
// The project root is resolved via resolveProjectRoot to prevent creating .ae/
...
func logTaskMetrics(input *HookInput) {
	...
	if projectRoot == "" {
		slog.Debug("task metrics: skipping, no AE project root found",
			"session_id", input.SessionID,
			"cwd", input.CWD,
		)
		return
	...
	logDir := filepath.Join(projectRoot, ".ae", "logs")
