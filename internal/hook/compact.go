...
import (
	...
	"github.com/AngeleyesTrue/ae-adk/internal/hook/memo"
)
	...
	if projectDir != "" {
		if err := memo.Write(projectDir, sections); err != nil {
			...
			slog.Info("pre-compact: session memo written",
				"path", filepath.Join(projectDir, ".ae/state/session-memo.md"),
			)
		}
	}
...
func buildCompactSections(input *HookInput, projectDir string) []memo.Section {
	...
	// P1: persistent mode status (read from .ae/state/persistent-mode.json).
	if projectDir != "" {
	...
	// P2: active worktrees (read from .ae/state/worktrees.json).
	if projectDir != "" {
...
// readPersistentMode reads .ae/state/persistent-mode.json and returns
// a human-readable summary, or empty string if the file does not exist.
func readPersistentMode(projectDir string) string {
	path := filepath.Join(projectDir, ".ae", "state", "persistent-mode.json")
...
// readWorktrees reads .ae/state/worktrees.json and returns a human-readable
// summary, or empty string if the file does not exist.
func readWorktrees(projectDir string) string {
	path := filepath.Join(projectDir, ".ae", "state", "worktrees.json")
	data, err := os.ReadFile(path)
	if err != nil {
		return ""
	}
