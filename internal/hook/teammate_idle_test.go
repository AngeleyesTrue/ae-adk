	...
	tests := []struct {
		...
		},
		{
			...
			setupDir: func(t *testing.T) string {
				t.Helper()
				dir := t.TempDir()
				// Create .ae/config/sections/ but no baseline file.
				if err := os.MkdirAll(filepath.Join(dir, ".ae", "config", "sections"), 0o755); err != nil {
					t.Fatal(err)
				}
				return dir
			},
...
func writeQualityConfig(t *testing.T, projectDir string, blockOnError bool) {
	t.Helper()
	dir := filepath.Join(projectDir, ".ae", "config", "sections")
...
func writeCoverageData(t *testing.T, projectDir string, percent float64) {
	t.Helper()
	stateDir := filepath.Join(projectDir, ".ae", "state")
