...
func (h *sessionEndHandler) Handle(ctx context.Context, input *HookInput) (*HookOutput, error) {
	...
	// This handles the case where the user ran 'ae glm' but ended the session
	// without running 'ae cc'. Without this, the stale GLM key persists and
...
// ANTHROPIC_AUTH_TOKEN is included: ae glm/cg sets it to the GLM API key in
...
// 'ae cc': the stale GLM key in settings.local.json would otherwise cause the
...
//   - If AE_BACKUP_AUTH_TOKEN exists, restore it as ANTHROPIC_AUTH_TOKEN.
//   - Otherwise, delete ANTHROPIC_AUTH_TOKEN (it was a GLM key, not OAuth).
//   - Always delete: AE_BACKUP_AUTH_TOKEN, ANTHROPIC_BASE_URL, and the three
...
func cleanupGLMSettingsLocal(projectDir string) {
	...
	if backup, ok := env["AE_BACKUP_AUTH_TOKEN"]; ok && backup != "" {
		env["ANTHROPIC_AUTH_TOKEN"] = backup
	} else {
		delete(env, "ANTHROPIC_AUTH_TOKEN")
	}

	delete(env, "AE_BACKUP_AUTH_TOKEN")
