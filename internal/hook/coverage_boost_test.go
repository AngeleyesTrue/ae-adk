...
func TestEnsureRankSessionHandler_WithCredentials(t *testing.T) {
	...
	rankDir := filepath.Join(tempHome, ".ae", "rank")
...
func TestLogTaskMetrics_MkdirAllFails(t *testing.T) {
	...
	t.Cleanup(func() { _ = os.Chmod(aeDir, 0o755) })
	...
	logPath := filepath.Join(tmpDir, ".ae", "logs", "task-metrics.jsonl")
...
func TestLogTaskMetrics_OpenFileFails(t *testing.T) {
	...
	tmpDir := t.TempDir()
	logsDir := filepath.Join(tmpDir, ".ae", "logs")
