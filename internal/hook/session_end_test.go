...
// login issues. ae glm/cg sets ANTHROPIC_AUTH_TOKEN in the tmux session to the
...
func TestCleanupGLMSettingsLocal(t *testing.T) {
	...
	tests := []struct {
		...
		wantSonnet       bool
		wantOpus         bool
		wantBackupToken  bool // true means AE_BACKUP_AUTH_TOKEN should still be present
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, ok := env["AE_BACKUP_AUTH_TOKEN"]; ok != tt.wantBackupToken {
				t.Errorf("AE_BACKUP_AUTH_TOKEN present=%v, want present=%v", ok, tt.wantBackupToken)
			}
...
func TestSessionEndHandler_Handle_CleansGLMFromSettingsLocal(t *testing.T) {
	...
	if _, ok := env["AE_BACKUP_AUTH_TOKEN"]; ok {
		t.Error("AE_BACKUP_AUTH_TOKEN should have been removed")
	}
}
