	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				...
				writeFile(t, root, ".ae/manifest.json", `{"version":"1.0.0","deployed_at":"","files":{}}`)
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				...
				writeFile(t, root, ".ae/config/sections/broken.yaml", "invalid: yaml: ][")
			},
			wantValid:  false,
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				...
				writeFile(t, root, ".ae/manifest.json", "not json{{{")
			},
			wantValid:  false,
...
func TestBackupExistingProject(t *testing.T) {
	t.Run("backs up existing ae directory", func(t *testing.T) {
		...
		if dirExists(filepath.Join(root, ".ae")) {
			t.Error("original .ae/ should have been moved")
		}
...
func TestValidateAE_NonYAMLFilesSkipped(t *testing.T) {
	...
	writeFile(t, root, ".ae/config/sections/readme.txt", "not yaml\n")
	writeFile(t, root, "CLAUDE.md", "# test\n")
