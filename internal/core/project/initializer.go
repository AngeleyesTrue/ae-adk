...
// aeDirs lists the directories to create under .ae/.
var aeDirs = []string{
	"config/sections",
	"specs",
	"reports",
...
func (i *projectInitializer) Init(ctx context.Context, opts InitOptions) (*InitResult, error) {
	...
	i.logger.Info("initializing AE project",
	...
	// Step 1: Create .ae/ directory structure
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if err := i.createAEDirs(opts.ProjectRoot, result); err != nil {
		return nil, fmt.Errorf("create .ae/ structure: %w", err)
	}
	...
	// Templates include .ae/config/sections/*.yaml via .tmpl rendering
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	if i.deployer != nil {
...
// createAEDirs creates the .ae/ directory structure.
func (i *projectInitializer) createAEDirs(root string, result *InitResult) error {
	for _, dir := range aeDirs {
		dirPath := filepath.Clean(filepath.Join(root, defs.AEDir, dir))
		if err := os.MkdirAll(dirPath, defs.DirPerm); err != nil {
			return fmt.Errorf("mkdir %s: %w", dirPath, err)
		}
		result.CreatedDirs = append(result.CreatedDirs, filepath.Join(defs.AEDir, dir))
	}
	return nil
}
...
func (i *projectInitializer) generateConfigsFallback(opts InitOptions, result *InitResult) error {
	sectionsDir := filepath.Clean(filepath.Join(opts.ProjectRoot, defs.AEDir, defs.SectionsSubdir))
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.SectionsSubdir, defs.UserYAML))
...
`, tmplCtx.ConversationLanguage, tmplCtx.ConversationLanguageName,
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.SectionsSubdir, defs.LanguageYAML))
...
`, tmplCtx.DevelopmentMode, tmplCtx.EnforceQuality, tmplCtx.TestCoverageTarget)
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.SectionsSubdir, defs.QualityYAML))
...
`, tmplCtx.AutoClear, tmplCtx.PlanTokens, tmplCtx.RunTokens, tmplCtx.SyncTokens)
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.SectionsSubdir, defs.WorkflowYAML))
...
`, tmplCtx.Version, tmplCtx.Version)
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.SectionsSubdir, defs.SystemYAML))
...
`, tmplCtx.ProjectName, tmplCtx.CreatedAt, tmplCtx.Version)
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.SectionsSubdir, defs.ProjectYAML))
...
func buildClaudeMDContent(opts InitOptions) string {
	...
	b.WriteString("Configuration files are located in `.ae/config/sections/`.\n\n")
...
func (i *projectInitializer) initManifest(root string, result *InitResult) error {
	...
	manifestPath := filepath.Join(root, defs.AEDir, defs.ManifestJSON)
	...
	result.CreatedFiles = append(result.CreatedFiles, filepath.Join(defs.AEDir, defs.ManifestJSON))
	return nil
}
