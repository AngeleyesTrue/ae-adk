...
// requiredAEDirs lists the directories that must exist under .ae/.
var requiredAEDirs = []string{
	"config/sections",
	"specs",
	"reports",
	"state",
...
func (v *projectValidator) Validate(root string) (*ValidationResult, error) {
	...
	// Check if .ae/ already exists
	aeDir := filepath.Join(root, defs.AEDir)
	if dirExists(aeDir) {
		result.Valid = false
		result.Errors = append(result.Errors, "project already initialized: .ae/ directory exists. Use --force to reinitialize.")
	}
...
func (v *projectValidator) ValidateAE(root string) (*ValidationResult, error) {
	...
	if !dirExists(aeDir) {
		result.Valid = false
		result.Errors = append(result.Errors, ".ae/ directory not found. Run 'ae init' first.")
		return result, nil
	}

	// Check required .ae/ subdirectories
	for _, subdir := range requiredAEDirs {
		dirPath := filepath.Join(aeDir, subdir)
		if !dirExists(dirPath) {
			result.Valid = false
			result.Errors = append(result.Errors, fmt.Sprintf("missing required directory: .ae/%s", subdir))
		}
	}

	// Check YAML config files are parseable
	sectionsDir := filepath.Join(aeDir, defs.SectionsSubdir)
	if dirExists(sectionsDir) {
		v.validateYAMLFiles(sectionsDir, result)
	}

	// Check manifest.json is valid JSON
	manifestPath := filepath.Join(aeDir, defs.ManifestJSON)
...
func BackupExistingProject(root string) (string, error) {
	...
	if err := os.Rename(aeDir, backupDir); err != nil {
