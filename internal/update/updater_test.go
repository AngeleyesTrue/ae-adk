...
func TestUpdater_Download_Success(t *testing.T) {
	...
	dir := t.TempDir()
	u := NewUpdater(filepath.Join(dir, "ae"), http.DefaultClient)
...
func TestExtractBinary_UnsupportedFormat(t *testing.T) {
	...
	u := &updaterImpl{binaryPath: filepath.Join(dir, "ae")}
...
func TestExtractFromZip_BinaryNotFound(t *testing.T) {
	...
	u := &updaterImpl{binaryPath: filepath.Join(dir, "ae")}
	_, err := u.extractFromZip(archivePath, "ae")
	if err == nil {
...
func TestExtractFromTarGz_BinaryNotFound(t *testing.T) {
	...
	u := &updaterImpl{binaryPath: filepath.Join(dir, "ae")}
	_, err := u.extractFromTarGz(archivePath, "ae")
	if err == nil {
