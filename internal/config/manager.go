...
// @MX:NOTE: [AUTO] Merges file values, compiled defaults, and environment variable priorities. AE_CONFIG_DIR env var can override config directory.
// Load reads configuration from the project root's .ae/ directory.
...
func (m *ConfigManager) Load(projectRoot string) (*Config, error) {
	...
	configDir := filepath.Join(filepath.Clean(projectRoot), defs.AEDir)

	// Support AE_CONFIG_DIR environment variable override
	if envDir := os.Getenv("AE_CONFIG_DIR"); envDir != "" {
...
func (m *ConfigManager) Save() error {
	...
	sectionsDir := filepath.Join(filepath.Clean(m.root), defs.AEDir, defs.SectionsSubdir)
...
func (m *ConfigManager) Reload() error {
	...
	configDir := filepath.Join(filepath.Clean(m.root), defs.AEDir)
	if envDir := os.Getenv("AE_CONFIG_DIR"); envDir != "" {
		configDir = filepath.Clean(envDir)
	}
...
func applyEnvOverrides(cfg *Config) {
	if mode := os.Getenv("AE_DEVELOPMENT_MODE"); mode != "" {
		cfg.Quality.DevelopmentMode = models.DevelopmentMode(mode)
	}
	if level := os.Getenv("AE_LOG_LEVEL"); level != "" {
		cfg.System.LogLevel = level
	}
	if format := os.Getenv("AE_LOG_FORMAT"); format != "" {
		cfg.System.LogFormat = format
	}
	if noColor := os.Getenv("AE_NO_COLOR"); noColor == "true" || noColor == "1" {
