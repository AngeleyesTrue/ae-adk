...
// setupTestdataDir creates a .ae/config/sections structure under tempDir
...
func TestLoaderLoadedSections(t *testing.T) {
	...
	loader := NewLoader()
	_, err := loader.Load(filepath.Join(root, ".ae"))
	if err != nil {
		t.Fatalf("Load() error: %v", err)
	}
...
func TestLoaderMultipleLoads(t *testing.T) {
	...
	// First load
	cfg1, err := loader.Load(filepath.Join(root, ".ae"))
	if err != nil {
	...
	sectionsDir := filepath.Join(root, ".ae", "config", "sections")
	...
	cfg2, err := loader.Load(filepath.Join(root, ".ae"))
	if err != nil {
...
`)
	...
	if cfg.State.StateDir != ".ae/custom-state" {
		t.Errorf("State.StateDir: got %q, want %q", cfg.State.StateDir, ".ae/custom-state")
	}
