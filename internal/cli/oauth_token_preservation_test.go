...
// (e.g. an Anthropic API key) lose it after a ae glm → ae cc round-trip.
//
...
//
// Root cause:
//  1. ae glm writes ANTHROPIC_AUTH_TOKEN=<GLM_KEY> to .claude/settings.local.json,
//     overwriting any pre-existing API key stored there.
//  2. ae cc calls removeGLMEnv() which deletes ANTHROPIC_AUTH_TOKEN entirely,
...
// as AE_BACKUP_AUTH_TOKEN. When removing GLM env vars, restore the backup.
...
// ae glm overwrites a pre-existing ANTHROPIC_AUTH_TOKEN (OAuth token) in
...
func TestOAuthToken_GLMOverwritesOAuthToken(t *testing.T) {
	...
	// Simulate ae glm: inject GLM env vars into settings.local.json.
	...
	// AE_BACKUP_AUTH_TOKEN so it can be restored when ae cc runs.
	...
	backupToken := afterGLM.Env["AE_BACKUP_AUTH_TOKEN"]
	if backupToken != oauthToken {
		t.Errorf("AE_BACKUP_AUTH_TOKEN = %q, want original OAuth token %q\n"+
			"FIX NEEDED: injectGLMEnvForTeam() must back up existing ANTHROPIC_AUTH_TOKEN "+
			"as AE_BACKUP_AUTH_TOKEN before overwriting it with the GLM key",
			backupToken, oauthToken)
	}
}

// TestOAuthToken_CCDoesNotRestoreOAuthToken demonstrates the second part of the bug:
// ae cc deletes ANTHROPIC_AUTH_TOKEN without restoring the original OAuth token,
...
func TestOAuthToken_CCDoesNotRestoreOAuthToken(t *testing.T) {
	...
	// Simulate the state after ae glm has run:
	// - ANTHROPIC_AUTH_TOKEN = GLM key
	// - AE_BACKUP_AUTH_TOKEN = original OAuth token (set by the fix)
	oauthToken := "claude-oauth-token-abc123"
	glmKey := "glm-api-key-xyz789"
	settingsAfterGLM := SettingsLocal{
		Env: map[string]string{
			"ANTHROPIC_AUTH_TOKEN":   glmKey,
			"AE_BACKUP_AUTH_TOKEN": oauthToken,
	...
	// Simulate ae cc: remove GLM env vars.
	...
	// BUG DEMONSTRATED: after ae cc, ANTHROPIC_AUTH_TOKEN is gone entirely.
	...
	if authToken != oauthToken {
		t.Errorf("after ae cc: ANTHROPIC_AUTH_TOKEN = %q, want restored OAuth token %q\n"+
			"FIX NEEDED: removeGLMEnv() must restore AE_BACKUP_AUTH_TOKEN as ANTHROPIC_AUTH_TOKEN "+
			"when removing GLM env vars",
			authToken, oauthToken)
	}

	// After the fix: AE_BACKUP_AUTH_TOKEN should be cleaned up.
	backupToken := afterCC.Env["AE_BACKUP_AUTH_TOKEN"]
	if backupToken != "" {
		t.Errorf("after ae cc: AE_BACKUP_AUTH_TOKEN = %q, want empty (should be cleaned up after restore)", backupToken)
	}
}

// TestOAuthToken_FullCycle verifies the complete ae glm -> ae cc round-trip
...
func TestOAuthToken_FullCycle(t *testing.T) {
	...
	// Step 1: ae glm injects GLM env vars (should back up OAuth token).
	if err := injectGLMEnvForTeam(settingsPath, glmConfig, glmKey); err != nil {
		t.Fatalf("injectGLMEnvForTeam() error: %v", err)
	}
	...
	if afterGLM.Env["AE_BACKUP_AUTH_TOKEN"] != oauthToken {
		t.Errorf("step1: AE_BACKUP_AUTH_TOKEN = %q, want %q\n"+
			"FIX NEEDED: injectGLMEnvForTeam() must back up existing token",
			afterGLM.Env["AE_BACKUP_AUTH_TOKEN"], oauthToken)
	}

	// Step 2: ae cc removes GLM env vars (should restore OAuth token).
	if err := removeGLMEnv(settingsPath); err != nil {
		t.Fatalf("removeGLMEnv() error: %v", err)
	}
	...
	if _, exists := afterCC.Env["AE_BACKUP_AUTH_TOKEN"]; exists {
		t.Error("step2: AE_BACKUP_AUTH_TOKEN should be deleted after restore")
	}
...
func TestOAuthToken_NoExistingToken(t *testing.T) {
	...
	// Step 1: ae glm injects GLM key (no prior token to back up).
	if err := injectGLMEnvForTeam(settingsPath, glmConfig, glmKey); err != nil {
		t.Fatalf("injectGLMEnvForTeam() error: %v", err)
	}
	...
	if _, exists := afterGLM.Env["AE_BACKUP_AUTH_TOKEN"]; exists {
		t.Errorf("AE_BACKUP_AUTH_TOKEN should NOT be set when there was no prior ANTHROPIC_AUTH_TOKEN, got: %q",
			afterGLM.Env["AE_BACKUP_AUTH_TOKEN"])
	}
	...
	// Step 2: ae cc removes GLM key (no backup to restore).
	if err := removeGLMEnv(settingsPath); err != nil {
		t.Fatalf("removeGLMEnv() error: %v", err)
	}
