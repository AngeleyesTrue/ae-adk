...
var rankCmd = &cobra.Command{
	Use:     "rank",
	Short:   "AE Rank leaderboard management",
	GroupID: "tools",
	Long:    "Manage AE Rank leaderboard: authenticate, view rankings, sync metrics, and configure exclusions.",
}
...
func newRankLoginCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "login",
		Short: "Authenticate with AE Cloud",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Start OAuth flow.
			_, _ = fmt.Fprintln(out, "Opening browser for AE Cloud authentication...")
...
func newRankStatusCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			out := cmd.OutOrStdout()
			if deps == nil {
				_, _ = fmt.Fprintln(out, "Rank client not configured. Run 'ae rank login' first.")
				return nil
			}
			...
			_, _ = fmt.Fprintln(out, renderCard("AE Rank", renderKeyValueLines(pairs)))
			return nil
		},
	}
}
...
func newRankLogoutCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			_, _ = fmt.Fprintln(out, renderSuccessCard("Logged out from AE Cloud"))
			return nil
		},
	}
}
...
func newRankSyncCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:    "sync",
		Short:  "Sync metrics to AE Cloud",
		Hidden: false,
		RunE: func(cmd *cobra.Command, args []string) error {
			out := cmd.OutOrStdout()
			if deps == nil {
			...
			if err := deps.EnsureRank(); err != nil {
				_, _ = fmt.Fprintln(out, "Not logged in. Run 'ae rank login' first.")
				return nil
			}
			...
			_, _ = fmt.Fprintln(out, "Syncing metrics to AE Cloud...")
...
func newRankExcludeCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long:   "Add a glob pattern to exclude from metrics sync. Patterns are stored in ~/.ae/config/rank.yaml.",
		Hidden: false,
...
func newRankIncludeCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long:   "Add a glob pattern to include in metrics sync. Patterns are stored in ~/.ae/config/rank.yaml.",
		Hidden: false,
		Args:   cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			out := cmd.OutOrStdout()
...
func newRankRegisterCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short:  "Register organization with AE Cloud",
		Hidden: true,
		Args:   cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			out := cmd.OutOrStdout()
...
// This enables automatic session metrics submission to AE Rank across all projects.
// It deploys a wrapper script to ~/.claude/hooks/ae-rank-session-end.sh (global)
...
func installRankHook() error {
	...
	for _, group := range sessionEndHooks {
		for _, h := range group.Hooks {
			// Check for either old name (ae-rank-session-end.sh) or new name (rank-submit.sh)
			if strings.Contains(h.Command, "ae-rank-session-end.sh") || strings.Contains(h.Command, "rank-submit.sh") {
				hookExists = true
				break
			}
		}
...
// to ~/.claude/hooks/. This global hook submits session metrics to AE Rank.
...
# AE Rank Session Submission Hook - Generated by ae-adk
# This script forwards stdin JSON to the ae hook session-end command.
# Location: ~/.claude/hooks/rank-submit.sh
# Scope: Global (applies to all projects)
# Action: Submits session metrics to AE Rank leaderboard
...
if command -v ae &> /dev/null; then
	exec ae hook session-end < "$temp_file"
fi

# Try detected Go bin path from initialization
if [ -f "%s/ae" ]; then
	exec "%s/ae" hook session-end < "$temp_file"
fi

# Try default ~/go/bin/ae
if [ -f "%s/go/bin/ae" ]; then
	exec "%s/go/bin/ae" hook session-end < "$temp_file"
fi
...
`, goBinPath, goBinPath, homeDir, homeDir)
	...
	// Use action-based name: rank-submit.sh instead of ae-rank-session-end.sh
...
func removeRankHook() error {
	...
	for _, group := range sessionEndHooks {
		...
		for _, h := range group.Hooks {
			...
			// Check for both old name (ae-rank-session-end.sh) and new name (rank-submit.sh)
			if !strings.Contains(h.Command, "ae-rank-session-end.sh") && !strings.Contains(h.Command, "rank-submit.sh") {
