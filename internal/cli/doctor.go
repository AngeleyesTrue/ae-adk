	...
	if fix && failCount > 0 {
		...
		for _, c := range checks {
			if c.Status == CheckFail {
				fixes = append(fixes, fmt.Sprintf("- %s: run 'ae init' to initialize project", c.Name))
			}
		}
...
func checkAEConfig(verbose bool) DiagnosticCheck {
	...
	aeDir := filepath.Join(cwd, defs.AEDir)
	info, err := os.Stat(aeDir)
	if err != nil || !info.IsDir() {
		check.Status = CheckWarn
		check.Message = ".ae/ directory not found (run 'ae init')"
		return check
	}

	configDir := filepath.Join(aeDir, defs.SectionsSubdir)
	if _, statErr := os.Stat(configDir); statErr != nil {
		check.Status = CheckWarn
		check.Message = ".ae/config/sections/ not found"
		return check
	}
	...
	if verbose {
		check.Detail = fmt.Sprintf("path: %s", aeDir)
	}
	return check
}
