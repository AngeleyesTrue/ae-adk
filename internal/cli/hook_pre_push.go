...
// Priority: AE_GIT_CONVENTION env var > config > default "auto".
func resolveConventionName() string {
	if envVal := os.Getenv("AE_GIT_CONVENTION"); envVal != "" {
		return envVal
	}
...
// Priority: AE_ENFORCE_ON_PUSH env var > config > default false.
func isEnforceOnPushEnabled() bool {
	if envVal := os.Getenv("AE_ENFORCE_ON_PUSH"); envVal != "" {
