...
Prerequisites:
  1. A GLM API key configured via 'ae glm setup <api-key>'
  2. Running inside a tmux session for pane-level environment isolation

Examples:
  ae glm setup sk-xxx    # First: save API key (one-time)
  ae cg                  # Then: launch hybrid mode
  ae cg -p work          # Use 'work' profile with hybrid mode

Use 'ae cc' to switch back to Claude-only mode.
Use 'ae glm' for all-GLM mode.`,
	GroupID:            "launch",
