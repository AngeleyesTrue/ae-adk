...
func TestRunStatus_NoAEDir(t *testing.T) {
	tmpDir := t.TempDir()
...
func TestRunStatus_WithAEDir(t *testing.T) {
	tmpDir := t.TempDir()
	...
	if err := os.MkdirAll(filepath.Join(tmpDir, ".ae", "specs", "SPEC-001"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(tmpDir, ".ae", "specs", "SPEC-002"), 0o755); err != nil {
		t.Fatal(err)
	}
	// Create config files
	...
	if err := os.WriteFile(filepath.Join(tmpDir, ".ae", "config", "sections", "quality.yaml"), []byte("constitution:\n  development_mode: ddd\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestRunStatus_OutputContainsProjectName(t *testing.T) {
	...
	if !strings.Contains(output, "ae-adk") {
		t.Errorf("output should contain 'ae-adk', got %q", output)
	}
}
