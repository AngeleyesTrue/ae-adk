...
func TestRootCmd_Use(t *testing.T) {
	if rootCmd.Use != "ae" {
		t.Errorf("rootCmd.Use = %q, want %q", rootCmd.Use, "ae")
	}
}
...
func TestRootCmd_Short(t *testing.T) {
	...
	if !strings.Contains(rootCmd.Short, "AE-ADK") {
		t.Errorf("rootCmd.Short should contain 'AE-ADK', got %q", rootCmd.Short)
	}
}
...
func TestRootCmd_NoArgs_ShowsHelp(t *testing.T) {
	...
	if !strings.Contains(output, "AE-ADK") {
		t.Error("root with no args should display help containing AE-ADK")
	}
}
