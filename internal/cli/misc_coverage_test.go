...
func TestResolveConventionName_EnvVariousValues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("AE_GIT_CONVENTION", tt.envVal)
...
func TestRemoveGLMEnv_WithGLMVars(t *testing.T) {
	...
	t.Setenv("AE_TEST_GLM_KEY", "test-token")
...
func TestEnsureUpdate_LocalSource(t *testing.T) {
	t.Setenv("AE_UPDATE_SOURCE", "local")
	t.Setenv("AE_RELEASES_DIR", t.TempDir())
...
func TestEnsureUpdate_CustomURL(t *testing.T) {
	t.Setenv("AE_UPDATE_SOURCE", "")
	t.Setenv("AE_UPDATE_URL", "https://api.example.com/releases")
