...
func TestInitCmd_PositionalArgCreatesDirectory(t *testing.T) {
	...
	// Verify .ae/ was created inside the new directory
	aeDir := filepath.Join(projectDir, ".ae")
	if _, statErr := os.Stat(aeDir); os.IsNotExist(statErr) {
		t.Error("expected .ae/ directory to be created inside project folder")
	}
...
func TestInitCmd_DotArgUsesCurrentDirectory(t *testing.T) {
	...
	// Verify .ae/ was created in current directory (not in a new "." folder)
	aeDir := filepath.Join(root, ".ae")
	if _, statErr := os.Stat(aeDir); os.IsNotExist(statErr) {
		t.Error("expected .ae/ directory to be created in current directory")
	}
