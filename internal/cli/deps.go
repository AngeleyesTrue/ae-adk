...
// wiring for the AE-ADK CLI. This file defines the Dependencies struct
...
func (d *Dependencies) EnsureUpdate() error {
	...
	// - AE_UPDATE_SOURCE=local: use local file-based releases
	// - AE_UPDATE_URL: custom GitHub API URL
	// - Default: GitHub releases based on version
	currentVersion := version.GetVersion()
	updateSource := os.Getenv("AE_UPDATE_SOURCE")
	...
	if updateSource == "local" {
		...
		localConfig := update.LocalConfig{
			ReleasesDir:    os.Getenv("AE_RELEASES_DIR"),
	...
	apiURL := os.Getenv("AE_UPDATE_URL")
	if apiURL == "" {
