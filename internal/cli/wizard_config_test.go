...
// setupSectionsDir creates the .ae/config/sections/ directory tree in a temp dir
// and returns the temp dir root path.
func setupSectionsDir(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	sectionsDir := filepath.Join(root, defs.AEDir, defs.SectionsSubdir)
...
func TestApplyWizardConfig_GitHubUsername(t *testing.T) {
	...
	userPath := filepath.Join(root, defs.AEDir, defs.SectionsSubdir, defs.UserYAML)
...
func TestApplyWizardConfig_GitStrategyYAML_NoFileCreatedWhenEmpty(t *testing.T) {
	...
	gitStratPath := filepath.Join(root, defs.AEDir, defs.SectionsSubdir, defs.GitStrategyYAML)
...
func TestApplyWizardConfig_QualityYAML_NoFileCreatedWhenEmpty(t *testing.T) {
	...
	qualityPath := filepath.Join(root, defs.AEDir, defs.SectionsSubdir, defs.QualityYAML)
