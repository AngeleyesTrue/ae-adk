...
func TestInjectGLMEnv_Success(t *testing.T) {
	...
	aeDir := filepath.Join(tmpHome, ".ae")
	if err := os.MkdirAll(aeDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(
		filepath.Join(aeDir, ".env.glm"),
...
func TestGetGLMEnvPath_ReturnsExpectedPath(t *testing.T) {
	...
	got := getGLMEnvPath()
	expected := filepath.Join(tmpHome, ".ae", ".env.glm")
	if got != expected {
...
func TestSaveGLMKey_FileContainsHeader(t *testing.T) {
	...
	if !strings.Contains(contentStr, "Generated by ae glm") {
...
func TestIsTestEnvironment_WithEnvVar(t *testing.T) {
	...
	if !isTestEnvironment() {
		t.Error("isTestEnvironment() should return true when AE_TEST_MODE=1")
	}
}

func TestIsTestEnvironment_WithEnvVarNotSet(t *testing.T) {
	// Even without AE_TEST_MODE, running under go test should detect test env.
...
func TestLoadGLMKey_ValidFile(t *testing.T) {
	...
	envDir := filepath.Join(tmpHome, ".ae")
