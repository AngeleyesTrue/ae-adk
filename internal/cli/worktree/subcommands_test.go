...
func TestRunNew_SpecID(t *testing.T) {
	...
	for _, cmd := range WorktreeCmd.Commands() {
		if cmd.Name() == "new" {
			...
			wantPath := filepath.Join(fakeHomeDir, ".ae", "worktrees", "test-project", "SPEC-AUTH-001")
...
func TestRunNew_DefaultPath(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    "feature-x",
			wantPath: filepath.Join(fakeHomeDir, ".ae", "worktrees", "test-project", "feature-x"),
		},
		{
			name:     "SPEC with different category uses global path",
			input:    "SPEC-UI-042",
			wantPath: filepath.Join(fakeHomeDir, ".ae", "worktrees", "test-project", "SPEC-UI-042"),
		},
		{
			name:     "branch with slash uses global path",
			input:    "fix/something",
			wantPath: filepath.Join(fakeHomeDir, ".ae", "worktrees", "test-project", "fix", "something"),
		},
	}
...
func TestGlobalPath_Base_MatchesSpecID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.specID, func(t *testing.T) {
			...
			globalPath := filepath.Join("/home/user", ".ae", "worktrees", "my-project", tt.specID)
