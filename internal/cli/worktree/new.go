...
// Overridable in tests.
var legacyWorktreeDir = filepath.Join(".", ".ae", "worktrees")
...
to branch names using the feature/ prefix convention.`,
	...
	cmd.Flags().String("path", "", "Custom path for the worktree (default: .ae/worktrees/<SPEC-ID> for SPEC IDs, ../<branch-name> otherwise)")
...
func runNew(cmd *cobra.Command, args []string) error {
	...
	if wtPath == "" {
		...
		wtPath = filepath.Join(homeDir, ".ae", "worktrees", projectName, specID)
	...
	if dirHasEntries(legacyWorktreeDir) {
		_, _ = fmt.Fprintln(cmd.ErrOrStderr(), "Warning: Legacy worktrees detected in .ae/worktrees/. Consider moving to ~/.ae/worktrees/{Project}/.")
	}
...
//
// Args:
//   - projectName: the project name (e.g., "ae-adk-go")
