...
func TestNewWorktreeWithTmuxCreation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedPath := filepath.Join(tempDir, ".ae", "worktrees", "test-project", tt.specID)
...
func TestWorktreeAutoCleanup(t *testing.T) {
	// R4: automatically run `ae worktree done SPEC-XXX` after PR merge
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			worktreePath := filepath.Join(tempDir, ".ae", "worktrees", "test-project", tt.specID)
