...
func TestRunTemplateSync_GetVersionError_ContinuesSync(t *testing.T) {
	// Create temp directory without .ae/config (to trigger error)
	tmpDir := t.TempDir()
...
func TestClassifyFileRisk(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			filename: ".ae/config/config.yaml",
			exists:   true,
			want:     "medium",
		},
		{
...
func TestIsMoaiManaged(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: true, // .ae/config/ is now managed by AE-ADK
		},
		{
			name: "claude md",
			path: "CLAUDE.md",
...
func TestBackupMoaiConfig_CreateBackup(t *testing.T) {
	...
	foundSystem := slices.Contains(metadata.BackedUpItems, ".ae/config/sections/system.yaml")
	if !foundSystem {
...
func TestBackupMetadata_Structure(t *testing.T) {
	...
	metadata := BackupMetadata{
		...
		BackedUpItems: []string{".ae/config/config.yaml", ".ae/config/settings.yaml"},
...
func TestEnsureGlobalSettingsEnv_RemovesGlobalHooksDir(t *testing.T) {
	...
	t.Run("RemovesExistingGlobalHooksMoaiDir", func(t *testing.T) {
		...
		if _, err := os.Stat(hooksDir); !os.IsNotExist(err) {
			t.Error("~/.claude/hooks/ae should be removed by ensureGlobalSettingsEnv")
		}
	})
...
func TestShouldSkipBinaryUpdate_EnvVar(t *testing.T) {
	origVal := os.Getenv("AE_SKIP_BINARY_UPDATE")
	defer func() {
		if origVal == "" {
			_ = os.Unsetenv("AE_SKIP_BINARY_UPDATE")
		} else {
			_ = os.Setenv("AE_SKIP_BINARY_UPDATE", origVal)
		}
	}()

	_ = os.Setenv("AE_SKIP_BINARY_UPDATE", "1")
	...
	if !got {
		t.Error("shouldSkipBinaryUpdate should return true when AE_SKIP_BINARY_UPDATE=1")
	}
}
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T, root string) {
				...
				mkDir(t, root, filepath.Join(".claude", "commands", "ae"))
				...
				mkFile(t, root, filepath.Join(".ae", "config", "config.yaml"), "version: 1")
				mkFile(t, root, filepath.Join(".ae", "config", "sections", "user.yaml"), "user: name")
			},
			verify: func(t *testing.T, root string, output string) {
				...
				if pathExists(root, filepath.Join(".claude", "commands", "ae")) {
					t.Error(".claude/commands/ae should have been removed")
				}
				...
				if pathExists(root, filepath.Join(".claude", "output-styles", "ae")) {
					t.Error(".claude/output-styles/ae should have been removed")
				}
				if pathExists(root, filepath.Join(".claude", "hooks", "ae")) {
					t.Error(".claude/hooks/ae should have been removed")
				}
				// entire .ae/config/ should be removed (backup handles preservation)
				if pathExists(root, filepath.Join(".ae", "config", "config.yaml")) {
					t.Error(".ae/config/config.yaml should have been removed")
				}
				if pathExists(root, filepath.Join(".ae", "config", "sections", "user.yaml")) {
					t.Error(".ae/config/sections/user.yaml should have been removed")
				}
			},
		},
		{
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				mkFile(t, root, filepath.Join(".ae", "config", "sections", "user.yaml"), "user: test")
				mkFile(t, root, filepath.Join(".ae", "config", "sections", "language.yaml"), "lang: ko")
				mkFile(t, root, filepath.Join(".ae", "config", "config.yaml"), "version: 2")
				mkFile(t, root, filepath.Join(".ae", "config", "backup_metadata.json"), `{"ts":"now"}`)
			},
			verify: func(t *testing.T, root string, output string) {
				// Entire .ae/config/ should be deleted (backup already done by Backup step)
				if pathExists(root, filepath.Join(".ae", "config", "sections")) {
					t.Error(".ae/config/sections/ should have been removed")
				}
				if pathExists(root, filepath.Join(".ae", "config", "sections", "user.yaml")) {
					t.Error(".ae/config/sections/user.yaml should have been removed")
				}
				if pathExists(root, filepath.Join(".ae", "config", "sections", "language.yaml")) {
					t.Error(".ae/config/sections/language.yaml should have been removed")
				}
				if pathExists(root, filepath.Join(".ae", "config", "config.yaml")) {
					t.Error(".ae/config/config.yaml should have been removed")
				}
				if pathExists(root, filepath.Join(".ae", "config", "backup_metadata.json")) {
					t.Error(".ae/config/backup_metadata.json should have been removed")
				}
			},
		},
		{
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				...
				mkFile(t, root, filepath.Join(".claude", "settings.json"), `{}`)
				mkDir(t, root, filepath.Join(".claude", "agents", "ae"))
				// commands/ae does not exist - should produce "Skipped"
			},
			verify: func(t *testing.T, root string, output string) {
...
func TestMigrateLegacyMemoryDir(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				legacyDir := filepath.Join(root, ".ae", "memory")
			...
			verify: func(t *testing.T, root string, output string) {
				if pathExists(root, filepath.Join(".ae", "memory")) {
					t.Error("legacy .ae/memory/ should have been removed")
				}
				if !pathExists(root, filepath.Join(".ae", "state")) {
					t.Error(".ae/state/ should exist after migration")
				}
				if !pathExists(root, filepath.Join(".ae", "state", "coverage.json")) {
					t.Error("coverage.json should have been moved to .ae/state/")
				}
		...
		},
		{
			...
			setup: func(t *testing.T, root string) {
				legacyDir := filepath.Join(root, ".ae", "memory")
				stateDir := filepath.Join(root, ".ae", "state")
				if err := os.MkdirAll(legacyDir, 0o755); err != nil {
					t.Fatal(err)
				}
			...
			verify: func(t *testing.T, root string, output string) {
				...
				if !pathExists(root, filepath.Join(".ae", "state", "new.json")) {
					t.Error(".ae/state/new.json should be preserved")
				}
				if pathExists(root, filepath.Join(".ae", "state", "old.json")) {
