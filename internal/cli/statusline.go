...
func runStatusline(cmd *cobra.Command, _ []string) error {
	...
	mode := statusline.StatuslineMode(os.Getenv("AE_STATUSLINE_MODE"))
	...
	opts := statusline.Options{
		Mode:          mode,
		NoColor:       os.Getenv("NO_COLOR") != "" || os.Getenv("AE_NO_COLOR") != "",
		RootDir:       projectRoot,
...
// .ae/config/sections/statusline.yaml. Returns nil if the file is missing,
...
func loadStatuslineFileConfig(projectRoot string) *statuslineFileConfig {
	...
	configPath := filepath.Join(projectRoot, ".ae", "config", "sections", "statusline.yaml")
...
// .ae/config/sections/statusline.yaml and returns a map of segment keys
