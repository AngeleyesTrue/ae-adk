...
func runStatus(cmd *cobra.Command, _ []string) error {
	...
	pairs := []kvPair{
		{"Project", projectName},
		{"Path", cwd},
		{"ADK", "ae-adk " + version.GetVersion()},
	}

	// Check .ae/ directory
	aeDir := filepath.Join(cwd, ".ae")
	if _, statErr := os.Stat(aeDir); statErr != nil {
		pairs = append(pairs,
			kvPair{"Status", "Not initialized (run 'ae init')"},
		)
		_, _ = fmt.Fprintln(out, renderCard("Project Status", renderKeyValueLines(pairs)))
		return nil
	}
	pairs = append(pairs, kvPair{"Config", filepath.Join(".ae", "config", "sections")})

	// Count SPECs
	specsDir := filepath.Join(aeDir, "specs")
