...
func TestCleanupMoaiWorktrees_GlobalPath(t *testing.T) {
	tests := []struct {
		...
		createGlobal bool // create a worktree under ~/.ae/worktrees/myproject/
		wantSubstrs  []string
		wantEmpty    bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.createGlobal {
				globalWorktreeDir := filepath.Join(tmpHome, ".ae", "worktrees", "myproject")
...
func TestUnifiedLaunch_CG_WithTestMode(t *testing.T) {
	...
	err := unifiedLaunch("", "claude_glm", nil)
	if err != nil {
		t.Fatalf("CG mode with AE_TEST_MODE=1 should not error, got: %v", err)
	}
}
...
func TestUnifiedLaunch_NotInProject(t *testing.T) {
	...
	if err == nil {
		t.Fatal("unifiedLaunch should error when not in a AE project")
	}
	if !strings.Contains(err.Error(), "not in a AE project") {
		t.Errorf("error should mention 'not in a AE project', got: %v", err)
	}
}
