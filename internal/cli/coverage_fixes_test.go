...
func TestCleanMoaiManagedPaths_ConfigDirRemoved(t *testing.T) {
	...
	configDir := filepath.Join(root, defs.AEDir, defs.ConfigSubdir)
	...
	if _, err := os.Stat(configDir); !os.IsNotExist(err) {
		t.Error(".ae/config directory should be removed")
	}
}
...
func TestCleanMoaiManagedPaths_OutputStylesRemoved(t *testing.T) {
	...
	outputStylesDir := filepath.Join(root, defs.ClaudeDir, defs.OutputStylesSubdir, "ae")
	...
	if _, err := os.Stat(outputStylesDir); !os.IsNotExist(err) {
		t.Error("output-styles/ae directory should be removed")
	}
}
...
func TestResolveConventionName_FromConfig(t *testing.T) {
	...
	t.Setenv("AE_GIT_CONVENTION", "")

	// Create a real ConfigManager with a loaded config
	tmpDir := t.TempDir()
	sectionsDir := filepath.Join(tmpDir, defs.AEDir, defs.SectionsSubdir)
...
func TestResolveConventionName_EnvOverridesConfig(t *testing.T) {
	...
	t.Setenv("AE_GIT_CONVENTION", "karma")
	got := resolveConventionName()
	if got != "karma" {
...
func TestRunPrePush_EnabledWithNoStdin(t *testing.T) {
	...
	t.Setenv("AE_GIT_CONVENTION", "conventional-commits")
