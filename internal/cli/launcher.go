...
func applyCCMode(root string) error {
	...
	worktreeMsg := cleanupAeWorktrees(root)
	if worktreeMsg != "" {
...
func applyCGMode(root, profileName string) error {
	...
	if !inTmux && os.Getenv("AE_TEST_MODE") != "1" {
		return fmt.Errorf("CG mode requires a tmux session.\n\n" +
			...
			"  tmux new -s ae\n" +
			"  ae cg\n\n" +
			"Or use 'ae glm' for all-GLM mode (no tmux required)")
	}
...
func removeGLMEnv(settingsPath string) error {
	...
	if settings.Env != nil {
		...
		if backup, ok := settings.Env["AE_BACKUP_AUTH_TOKEN"]; ok && backup != "" {
			settings.Env["ANTHROPIC_AUTH_TOKEN"] = backup
			delete(settings.Env, "AE_BACKUP_AUTH_TOKEN")
		} else {
...
// cleanupAeWorktrees removes ae-related git worktrees from both the
// local .claude/worktrees/ path and the global ~/.ae/worktrees/*/ paths.
// These are worktrees created by /ae --team with names like worker-SPEC-XXX.
func cleanupAeWorktrees(projectRoot string) string {
	...
	for _, line := range lines {
		...
		for _, base := range basePaths {
			...
			// of whether it is under .claude/worktrees/ or ~/.ae/worktrees/.
