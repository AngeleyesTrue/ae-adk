...
This command:
  1. Loads GLM credentials from ~/.ae/.env.glm
...
Use 'ae glm setup <key>' to save your API key first.

Flags:
  -p, --profile <name>   Use a named Claude profile (~/.ae/claude-profiles/<name>/)

Examples:
  ae glm setup sk-xxx    # Save API key (one-time)
  ae glm                 # Launch with GLM backend
  ae glm -p work         # Use 'work' profile with GLM

For hybrid mode (Claude lead + GLM teammates), use 'ae cg' instead.
Use 'ae cc' to switch back to Claude backend.`,
	GroupID:            "launch",
...
var glmStatusCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		if key == "" {
			...
			_, _ = fmt.Fprintln(cmd.OutOrStdout(), "Run 'ae glm setup <api-key>' to save your key")
			return nil
		}
...
func enableTeamMode(cmd *cobra.Command, isHybrid bool) error {
	...
	if apiKey == "" {
		if isHybrid {
			return fmt.Errorf("GLM API key not found\n\n"+
				...
				"  1. ae glm setup <api-key>   (saves key to ~/.ae/.env.glm)\n"+
				"  2. ae cg                     (enable hybrid mode)\n\n"+
				"Or set the %s environment variable", glmConfig.EnvVar)
		}
		return fmt.Errorf("GLM API key not found. Run 'ae glm setup <api-key>' to save your key, or set %s environment variable", glmConfig.EnvVar)
	}
	...
	if isHybrid && !inTmux && os.Getenv("AE_TEST_MODE") != "1" {
		return fmt.Errorf("CG mode requires a tmux session: tmux is required for Claude + GLM hybrid mode because: Leader (this pane) uses Claude API, Teammates (new panes) inherit GLM env to use Z.AI API. Start a tmux session first: tmux new -s ae; ae cg. Or use 'ae glm' for all-GLM mode (no tmux required)")
	}
	...
	if isHybrid {
		...
		_, _ = fmt.Fprintln(out, renderSuccessCard(
			...
			"Config saved to: .ae/config/sections/llm.yaml",
			"",
			"How it works:",
			...
			"  2. Run workflow: /ae --team \"your task\"",
			"",
			"Run 'ae cc' to disable CG/GLM team mode.",
		))
	} else {
		...
		_, _ = fmt.Fprintln(out, renderSuccessCard(
			...
			"",
			"Next steps:",
			"  1. Ensure you're in a tmux session (tmux new -s ae)",
			"  2. Start Claude Code: claude",
			"  3. Run workflow: /ae --team \"your task\"",
			"",
			"Run 'ae cc' to disable GLM team mode.",
		))
	}
...
// Called when switching back to Claude mode (ae cc).
...
// persistTeamMode saves the team_mode value to .ae/config/sections/llm.yaml.
func persistTeamMode(projectRoot, mode string) error {
	sectionsDir := filepath.Join(filepath.Clean(projectRoot), defs.AEDir, defs.SectionsSubdir)
...
// AE_BACKUP_AUTH_TOKEN before being overwritten. removeGLMEnv restores it.
...
func injectGLMEnvForTeam(settingsPath string, glmConfig *GLMConfigFromYAML, apiKey string) error {
	...
	if existing := settings.Env["ANTHROPIC_AUTH_TOKEN"]; existing != "" && existing != apiKey {
		settings.Env["AE_BACKUP_AUTH_TOKEN"] = existing
	}
...
// getGLMEnvPath returns the path to ~/.ae/.env.glm.
func getGLMEnvPath() string {
	home, err := userHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".ae", ".env.glm")
}

// saveGLMKey saves the GLM API key to ~/.ae/.env.glm.
func saveGLMKey(key string) error {
	...
	content := fmt.Sprintf("# GLM API Key for AE-ADK\n# Generated by ae glm\nGLM_API_KEY=\"%s\"\n", escapedKey)
...
// loadGLMKey loads the GLM API key from ~/.ae/.env.glm.
func loadGLMKey() string {
	// Allow tests to simulate a specific GLM key without requiring a real
	// ~/.ae/.env.glm file. Only set this in test code via t.Setenv.
	if testKey := os.Getenv("AE_TEST_GLM_KEY"); testKey != "" {
		return testKey
	}
...
func isTestEnvironment() bool {
	if flag := os.Getenv("AE_TEST_MODE"); flag == "1" {
		return true
	}
...
func findProjectRoot() (string, error) {
	...
	for {
		...
		if homeDir != "" && dir == homeDir {
			return "", fmt.Errorf("not in a AE project (reached home directory)")
		}
		if _, err := os.Stat(filepath.Join(dir, ".ae")); err == nil {
			return dir, nil
		}
		parent := filepath.Dir(dir)
		if parent == dir {
			return "", fmt.Errorf("not in a AE project (no .ae directory found)")
		}
		dir = parent
	}
}
