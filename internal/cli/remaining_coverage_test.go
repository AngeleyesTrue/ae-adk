...
func TestRunInit_WithSubdirectoryArg_NonInteractive(t *testing.T) {
	...
	// Set AE_SKIP_BINARY_UPDATE so runInit doesn't attempt a network call.
...
// =============================================================================
// runCC — findProjectRoot fails when not in a AE project (cc.go:30)
// =============================================================================

func TestRunCC_FindProjectRootFails(t *testing.T) {
	// Change to a temp directory that has no .ae directory
	...
	err = runCC(cmd, nil)
	if err == nil {
		t.Fatal("runCC should error when not in a AE project")
	}
