...
// ae glm is the only writer of AUTH_TOKEN to project settings.local.json.
...
func TestRemoveGLMEnv_RemovesAuthTokenUnconditionally(t *testing.T) {
	...
	// ANTHROPIC_AUTH_TOKEN must be removed — ae glm is the only source for
...
func TestRemoveGLMEnv_GLMOnlyVarsResultsInNilEnv(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(aeDir, ".env.glm"), []byte("GLM_API_KEY=tok\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestRunInitWizard_NotInitialized(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "ae init") {
		t.Errorf("output should mention 'ae init', got: %q", buf.String())
	}
}
...
func TestDeployGlobalRankHookScript_Success(t *testing.T) {
	...
	if !strings.Contains(string(data), "ae hook session-end") {
		t.Error("script should call ae hook session-end")
	}
}
...
func TestGetGLMEnvPath_WithCustomHome(t *testing.T) {
	...
	expected := filepath.Join(tmpDir, ".ae", ".env.glm")
	if result != expected {
...
func TestIsEnforceOnPushEnabled_EnvOther(t *testing.T) {
	...
	deps = nil
	defer func() { deps = origDeps }()
	t.Setenv("AE_ENFORCE_ON_PUSH", "yes")
	got := isEnforceOnPushEnabled()
	if got {
...
// When AE_ENFORCE_ON_PUSH=true, the function proceeds past the early return
// =============================================================================
...
func TestRunPrePush_EnforcementEnabled_NoConvention(t *testing.T) {
	...
	// runPrePush will try to load convention from tmpDir (which has no .ae/).
...
func TestBackupMoaiConfig_PathIsFile(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(aeDir, "config"), []byte("file"), 0o644); err != nil {
		t.Fatal(err)
	}
...
// =============================================================================
// cleanupAeWorktrees — in a git repo, parses porcelain output
// (cc.go:160 — at 75.0%)
// =============================================================================
...
func TestNewRankExcludeCmd_WithPattern(t *testing.T) {
	...
	// May succeed or fail depending on whether ~/.ae/config/ is writable
...
// =============================================================================
// cleanupAeWorktrees — with a .claude/worktrees/worker-X in git output
