...
func TestCGCommandRegistered(t *testing.T) {
	...
	if glmFlag != nil {
		t.Error("glmCmd should NOT have a --hybrid flag (use 'ae cg' instead)")
	}
}
...
func TestEnableTeamModeInjectsGLMEnvAndTmux(t *testing.T) {
	...
	homeDir := t.TempDir()
	envGLMPath := filepath.Join(homeDir, ".ae", ".env.glm")
...
func TestEnableTeamModeCGRequiresAPIKey(t *testing.T) {
	...
	errMsg := err.Error()
	if !strings.Contains(errMsg, "ae glm setup <api-key>") {
		t.Errorf("error should mention 'ae glm <api-key>', got: %v", err)
	}
	if !strings.Contains(errMsg, "ae cg") {
		t.Errorf("error should mention 'ae cg', got: %v", err)
	}
}
...
func TestEnableTeamModeCGRequiresTmux(t *testing.T) {
	// NOTE: Do NOT set AE_TEST_MODE here. This test validates the tmux
	// requirement check (glm.go:111), which is bypassed by AE_TEST_MODE=1.
	...
	// Should suggest ae glm as alternative
	if !strings.Contains(errMsg, "ae glm") {
		t.Errorf("error should mention 'ae glm' alternative, got: %v", err)
	}
}
...
// TestCGAutoResetsGLMMode verifies that 'ae cg' automatically cleans up
// GLM env vars from settings.local.json when switching from 'ae glm' mode.
// This eliminates the need to run 'ae cc' between 'ae glm' and 'ae cg'.
func TestCGAutoResetsGLMMode(t *testing.T) {
	...
	// Step 1: Run 'ae glm' (all-GLM mode) - injects GLM env vars
	err := enableTeamMode(glmCmd, false)
	if err != nil {
	...
	if _, exists := glmSettings.Env["ANTHROPIC_BASE_URL"]; !exists {
		t.Fatal("after 'ae glm', settings.local.json should contain ANTHROPIC_BASE_URL")
	}

	// Step 2: Run 'ae cg' directly (without 'ae cc')
	err = enableTeamMode(cgCmd, true)
	if err != nil {
	...
	// (all removed by removeGLMEnv; GLM key is re-injected from ~/.ae/.env.glm by 'ae glm')
	for _, key := range []string{
		...
		if _, exists := cgSettings.Env[key]; exists {
			t.Errorf("after 'ae cg', settings.local.json should NOT contain %s (lead uses Claude)", key)
		}
	}
...
func TestCleanupMoaiWorktrees(t *testing.T) {
	...
	// cleanupAeWorktrees should handle non-git directories gracefully
	result := cleanupAeWorktrees(projectRoot)
	// Result should be empty since there's no git repo
	if result != "" {
		t.Logf("cleanupAeWorktrees returned: %s (expected empty for non-git dir)", result)
	}
}
