...
func TestPersistTeamMode_CreatesDirectoryAndFile(t *testing.T) {
	...
	llmPath := filepath.Join(tmpDir, ".ae", "config", "sections", "llm.yaml")
...
func TestDeployGlobalRankHookScript(t *testing.T) {
	...
	if !strings.Contains(content, "ae hook session-end") {
		t.Error("script should reference ae hook session-end")
	}
...
func TestRunPrePush_EnforcementDisabled(t *testing.T) {
	...
	t.Setenv("AE_ENFORCE_ON_PUSH", "false")
...
func TestResolveConventionName_EnvVar(t *testing.T) {
	t.Setenv("AE_GIT_CONVENTION", "angular")
...
func TestCleanMoaiManagedPaths_WithExistingPaths(t *testing.T) {
	...
	paths := []string{
		...
		filepath.Join(tmpDir, ".claude", "commands", "ae", "test.md"),
		filepath.Join(tmpDir, ".claude", "agents", "ae", "test.md"),
		filepath.Join(tmpDir, ".claude", "rules", "ae", "test.md"),
		filepath.Join(tmpDir, ".claude", "hooks", "ae", "test.sh"),
		filepath.Join(tmpDir, ".ae", "config", "sections", "user.yaml"),
	}
...
func TestRunCC_SuccessfulExecution(t *testing.T) {
	...
	// ANTHROPIC_AUTH_TOKEN is also removed — it is re-injected from ~/.ae/.env.glm by 'ae glm'
...
func TestLoadSegmentConfig_ValidFile(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, ".ae", "config", "sections", "statusline.yaml")
...
func TestEnableTeamMode_NoAPIKey_GLM(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, ".ae", "config", "sections", "user.yaml"), []byte("user:\n  name: test\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestEnableTeamMode_NoAPIKey_CG(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "ae glm") {
		t.Errorf("CG error should mention 'ae glm', got: %v", err)
	}
}
...
func TestRunPrePush_EnabledWithConvention(t *testing.T) {
	t.Setenv("AE_ENFORCE_ON_PUSH", "true")
	t.Setenv("AE_GIT_CONVENTION", "conventional")
...
func TestGetGLMEnvPath_Valid(t *testing.T) {
	...
	if !strings.Contains(path, ".ae") || !strings.Contains(path, ".env.glm") {
...
func TestRunCC_NoProjectRoot(t *testing.T) {
	...
	origFn := findProjectRootFn
	findProjectRootFn = func() (string, error) { return "", fmt.Errorf("not in a AE project (test)") }
	defer func() { findProjectRootFn = origFn }()
...
func TestRemoveGLMEnv_WithExistingGLMVars(t *testing.T) {
	...
	// ANTHROPIC_AUTH_TOKEN is also removed (re-injected from ~/.ae/.env.glm by 'ae glm')
	if strings.Contains(string(data), "ANTHROPIC_AUTH_TOKEN") {
		t.Error("ANTHROPIC_AUTH_TOKEN should be removed by removeGLMEnv")
	}
	if strings.Contains(string(data), "ANTHROPIC_BASE_URL") {
...
func TestCleanupMoaiWorktrees_RealGitRepo(t *testing.T) {
	...
	// No .ae/worktrees, should return empty
	result := cleanupAeWorktrees(tmpDir)
	if result != "" {
		t.Logf("cleanupAeWorktrees result: %q", result)
	}
}
...
func TestCleanMoaiManagedPaths_WithFiles(t *testing.T) {
	...
	dirs := []string{
		filepath.Join(tmpDir, ".claude", "commands", "ae"),
		filepath.Join(tmpDir, ".claude", "agents", "ae"),
		filepath.Join(tmpDir, ".claude", "rules", "ae"),
		filepath.Join(tmpDir, ".claude", "hooks", "ae"),
		filepath.Join(tmpDir, ".ae", "config", "sections"),
	}
...
func TestSaveGLMKey_WritesFile(t *testing.T) {
	...
	// Verify the file was created
	envPath := filepath.Join(tmpHome, ".ae", ".env.glm")
...
func TestRunInit_NonInteractive(t *testing.T) {
	...
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("AE_SKIP_BINARY_UPDATE", "1")
...
func TestRunTemplateSyncWithReporter_FullDeploy(t *testing.T) {
	...
	// Create .ae/manifest.json for manifest manager
	manifestPath := filepath.Join(tmpDir, ".ae", "manifest.json")
...
func TestCleanMoaiManagedPaths_WithExistingFiles(t *testing.T) {
	...
	paths := []string{
		...
		filepath.Join(tmpDir, ".claude", "agents", "ae", "test.md"),
		filepath.Join(tmpDir, ".claude", "rules", "ae", "test.md"),
		filepath.Join(tmpDir, ".claude", "output-styles", "ae", "test.md"),
		filepath.Join(tmpDir, ".claude", "hooks", "ae", "test.sh"),
		filepath.Join(tmpDir, ".ae", "config", "config.yaml"),
	}
...
func TestIsEnforceOnPushEnabled_EnvValues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.envVal, func(t *testing.T) {
			t.Setenv("AE_ENFORCE_ON_PUSH", tt.envVal)
...
func TestEnableTeamMode_WithAPIKey_Hybrid(t *testing.T) {
	...
	aeDir := filepath.Join(tmpDir, ".ae")
	sectionsDir := filepath.Join(aeDir, "config", "sections")
	claudeDir := filepath.Join(tmpDir, ".claude")
	if err := os.MkdirAll(sectionsDir, 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestGetGLMEnvPath_WithHOME(t *testing.T) {
	...
	path := getGLMEnvPath()
	if !strings.Contains(path, ".ae") || !strings.Contains(path, ".env.glm") {
		t.Errorf("expected path containing .ae/.env.glm, got: %s", path)
	}
}
...
func TestRunCC_FullPath(t *testing.T) {
	...
	t.Setenv("AE_TEST_GLM_KEY", "key")
	tmpDir := t.TempDir()

	// Create project structure
	aeDir := filepath.Join(tmpDir, ".ae", "config", "sections")
	claudeDir := filepath.Join(tmpDir, ".claude")
	...
	llmYAML := "llm:\n  team_mode: glm\n"
	if err := os.WriteFile(filepath.Join(aeDir, "llm.yaml"), []byte(llmYAML), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestSaveGLMKey_Permissions(t *testing.T) {
	...
	envPath := filepath.Join(tmpDir, ".ae", ".env.glm")
...
func TestBackupMoaiConfig_StatError(t *testing.T) {
	...
	configPath := filepath.Join(tmpDir, ".ae", "config")
	if err := os.MkdirAll(filepath.Dir(configPath), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestCleanMoaiManagedPaths_WithGlobMatches(t *testing.T) {
	...
	// Create hooks dir
	if err := os.MkdirAll(filepath.Join(tmpDir, ".claude", "hooks", "ae"), 0o755); err != nil {
		t.Fatal(err)
	}

	// Create rules dir
	if err := os.MkdirAll(filepath.Join(tmpDir, ".claude", "rules", "ae"), 0o755); err != nil {
		t.Fatal(err)
	}

	// Create output-styles dir
	if err := os.MkdirAll(filepath.Join(tmpDir, ".claude", "output-styles", "ae"), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestEnableTeamMode_NoAPIKey(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(aeDir, "llm.yaml"), []byte(llmContent), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestCleanupOldBackups_NonDirBackupPath(t *testing.T) {
	...
	// Create .ae/backups as a file (not a dir)
	backupsPath := filepath.Join(tmpDir, ".ae", "backups")
...
func TestCleanupOldBackups_InvalidPatterns(t *testing.T) {
	...
	backupsDir := filepath.Join(tmpDir, ".ae", "backups")
...
func TestShouldSkipBinaryUpdate_EnvVar_Phase5(t *testing.T) {
	...
	if !shouldSkipBinaryUpdate(cmd) {
		t.Error("expected shouldSkipBinaryUpdate to return true with AE_SKIP_BINARY_UPDATE=1")
	}
}
...
func TestIsTestEnvironment_WithFlag(t *testing.T) {
	t.Setenv("AE_TEST_MODE", "1")
	if !isTestEnvironment() {
		t.Error("expected isTestEnvironment to return true with AE_TEST_MODE=1")
	}
}
...
func TestInstallRankHook_Phase6(t *testing.T) {
	...
	if !strings.Contains(string(scriptData), "ae hook session-end") {
		t.Error("expected hook script to contain ae hook session-end")
	}
...
func TestDeployGlobalRankHookScript_Phase6(t *testing.T) {
	...
	if !strings.Contains(content, "ae hook session-end") {
		t.Error("expected ae hook command")
	}
...
func TestCleanMoaiManagedPaths_WithGlobAndDirs(t *testing.T) {
	...
	paths := []string{
		...
		filepath.Join(tmpDir, ".claude", "rules", "ae"),
		filepath.Join(tmpDir, ".claude", "hooks", "ae"),
		filepath.Join(tmpDir, ".ae", "config"),
	}
	...
	if _, err := os.Stat(filepath.Join(tmpDir, ".claude", "agents", "ae")); !os.IsNotExist(err) {
		t.Error("expected agents/ae to be removed")
	}
	if _, err := os.Stat(filepath.Join(tmpDir, ".ae", "config")); !os.IsNotExist(err) {
		t.Error("expected .ae/config to be removed")
	}
...
func TestLoadGLMKey_Phase6(t *testing.T) {
	...
	// Save first
	envDir := filepath.Join(tmpDir, ".ae")
...
func TestFindProjectRoot_NoMoaiDir(t *testing.T) {
	...
	if err == nil {
		t.Error("expected error when no .ae directory found")
	}
}
