...
func TestGLMCmd_AddsModelOverrides(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("ae glm should not error, got: %v", err)
	}
...
// TestCGCmd_DoesNotAddModelOverridesToSettings verifies that 'ae cg' does NOT add
...
func TestCGCmd_DoesNotAddModelOverridesToSettings(t *testing.T) {
	...
	t.Setenv("AE_TEST_MODE", "1") // Skip tmux requirement
	...
	if err != nil {
		t.Fatalf("ae cg should not error in test mode, got: %v", err)
	}
