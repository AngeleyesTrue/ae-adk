...
`
	...
	destPath := filepath.Join(aeDir, "status_line.sh")
	if err := os.WriteFile(destPath, []byte("# Template version\n"), 0644); err != nil {
		t.Fatalf("write template: %v", err)
	}

	backups := []fileBackup{
		{path: ".ae/status_line.sh", data: []byte(userContent)},
	}
...
func TestCollectMergeableFiles_ConfigSections(t *testing.T) {
	...
	// Create .ae/config/sections directory structure
	configSectionsDir := filepath.Join(tempDir, ".ae", "config", "sections")
	...
	expectedFiles := []string{".mcp.json", ".claude/settings.json", ".ae/status_line.sh"}
