...
// Cache is stored at ~/.ae/cache/usage.json (REQ-V3-API-002).
func NewUsageCollector(homeDir string) UsageProvider {
	cacheDir := filepath.Join(homeDir, ".ae", "cache")
...
// @MX:WARN: [AUTO] 백그라운드 goroutine 모니터링 - 취소 지원 필요
// @MX:REASON: 장시간 실행 goroutine, 적절한 종료 처리 없으면 리소스 누수 위험
