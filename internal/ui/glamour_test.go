...
func TestRenderMarkdown_NoColor_ReturnsUnchanged(t *testing.T) {
	_ = os.Setenv("AE_NO_COLOR", "true")
	defer func() { _ = os.Unsetenv("AE_NO_COLOR") }()
...
func TestNoColorFromEnv_AE_NO_COLOR_True(t *testing.T) {
	_ = os.Setenv("AE_NO_COLOR", "true")
	defer func() { _ = os.Unsetenv("AE_NO_COLOR") }()

	if !noColorFromEnv() {
		t.Error("expected true when AE_NO_COLOR=true")
	}
}

func TestNoColorFromEnv_AE_NO_COLOR_1(t *testing.T) {
	_ = os.Setenv("AE_NO_COLOR", "1")
	defer func() { _ = os.Unsetenv("AE_NO_COLOR") }()

	if !noColorFromEnv() {
		t.Error("expected true when AE_NO_COLOR=1")
	}
}

func TestNoColorFromEnv_AE_NO_COLOR_Other(t *testing.T) {
	_ = os.Setenv("AE_NO_COLOR", "false")
	defer func() { _ = os.Unsetenv("AE_NO_COLOR") }()

	if noColorFromEnv() {
		t.Error("expected false when AE_NO_COLOR=false")
	}
}
