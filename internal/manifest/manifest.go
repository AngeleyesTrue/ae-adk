...
type Manager interface {
	// Load reads the manifest from {projectRoot}/.ae/manifest.json.
...
// @MX:NOTE: [AUTO] 3개 파일에서 사용하는 매니페스트 매니저 생성자
...
// @MX:ANCHOR: [AUTO] 14개 파일에서 참조하는 매니페스트 로드 핵심 함수
// @MX:REASON: 변경 시 init, update, hook, config 등 전체 파일 추적에 영향
...
func (m *manifestManager) Load(projectRoot string) (*Manifest, error) {
	...
	m.manifestPath = filepath.Join(m.projectRoot, defs.AEDir, defs.ManifestJSON)
...
// @MX:ANCHOR: [AUTO] 4개 파일에서 참조하는 파일 추적 등록 함수
// @MX:REASON: 변경 시 deployer, model_policy의 파일 프로비넌스 추적 손상 위험
...
// @MX:ANCHOR: [AUTO] 3개 파일에서 참조하는 매니페스트 항목 조회 함수
// @MX:REASON: deployer가 파일 존재 여부 판단에 의존
