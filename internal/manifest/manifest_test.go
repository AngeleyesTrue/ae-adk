...
// setupProject creates a temporary project directory with .ae/ subdirectory.
func setupProject(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(dir, ".ae"), 0o755); err != nil {
...
func TestManagerLoad(t *testing.T) {
	...
	t.Run("corrupt_json_recovery", func(t *testing.T) {
		...
		corruptPath := filepath.Join(root, ".ae", manifestFileName+".corrupt")
		...
		origPath := filepath.Join(root, ".ae", manifestFileName)
...
func TestManagerSave(t *testing.T) {
	t.Run("save_and_reload", func(t *testing.T) {
		...
		manifestPath := filepath.Join(root, ".ae", manifestFileName)
	...
	t.Run("save_creates_directory", func(t *testing.T) {
		...
		path := filepath.Join(root, ".ae", manifestFileName)
...
func TestManagerSaveReadOnlyDir(t *testing.T) {
	t.Run("save_to_readonly_parent", func(t *testing.T) {
		...
		mgr := &manifestManager{
			...
			manifestPath: filepath.Join(readonlyDir, ".ae", manifestFileName),
			manifest:     NewManifest(),
		}
		mgr.manifest.Version = "1.0.0"
