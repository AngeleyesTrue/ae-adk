...
AE-ADK는 Claude Code를 위한 **고성능 AI 개발 환경**입니다. 27개 전문 AI 에이전트와 52개 스킬이 협력하여 품질 있는 코드를 만듭니다. 신규 프로젝트와 기능 개발에는 TDD(기본값)를, 테스트 커버리지가 낮은 기존 프로젝트에는 DDD를 자동 적용하며, Sub-Agent와 Agent Teams 이중 실행 모드를 지원합니다.
...
Python 기반 AE-ADK(~73,000줄)를 Go로 완전히 재작성했습니다.
...
AE-ADK는 **하네스 엔지니어링(Harness Engineering)** 패러다임을 구현합니다 — 코드를 직접 작성하는 대신 AI 에이전트를 위한 환경을 설계합니다.

| 구성 요소 | 설명 | 명령어 |
|-----------|------|--------|
| **자가 검증 루프** | 에이전트가 코드 작성 → 테스트 → 실패 → 수정 → 통과 사이클을 자율적으로 반복 | `/ae loop` |
| **컨텍스트 맵** | 코드베이스 아키텍처 맵과 문서를 에이전트가 항상 참조 가능 | `/ae codemaps` |
| **세션 지속성** | `progress.md`가 완료된 단계를 추적; 중단된 실행이 자동으로 재시작 | `/ae run SPEC-XXX` |
| **실패 체크리스트** | 모든 인수 조건이 실행 시작 시 대기(pending) 작업으로 등록; 구현되면 완료로 표시 | `/ae run SPEC-XXX` |
| **언어 중립** | 16개 언어 지원: 언어 자동 감지, 적합한 LSP/린터/테스트/커버리지 도구 자동 선택 | 전체 워크플로우 |
| **가비지 컬렉션** | 죽은 코드, AI Slop, 미사용 임포트 주기적 스캔 및 제거 | `/ae clean` |
| **스캐폴딩 우선** | 구현 전 빈 파일 스텁 생성으로 엔트로피 방지 | `/ae run SPEC-XXX` |
...
> 프리빌트 바이너리는 [Releases](https://github.com/modu-ai/ae-adk/releases) 페이지에서 다운로드할 수 있습니다.
...
```bash
# Claude Code 실행 후
/ae project                            # 프로젝트 문서 생성 (product.md, structure.md, tech.md)
/ae plan "사용자 인증 기능 추가"        # SPEC 문서 생성
/ae run SPEC-AUTH-001                   # DDD/TDD 구현
/ae sync SPEC-AUTH-001                  # 문서 동기화 & PR 생성
/ae github issues                      # GitHub 이슈 자동화 (Agent Teams)
/ae github pr 123                       # PR 다각도 검토 (multi-perspective)
```
...
AE-ADK는 프로젝트 상태에 따라 최적의 개발 방법론을 자동 선택합니다.
...
> 방법론은 `ae init` 시 자동 선택되며 (`--mode <ddd|tdd>`, 기본값: tdd), `.ae/config/sections/quality.yaml`의 `development_mode`에서 변경할 수 있습니다.
>
> **참고**: AE-ADK v2.5.0+는 이진 방법론 선택(TDD 또는 DDD만)을 사용합니다. 명확성과 일관성을 위해 hybrid 모드는 제거되었습니다.

### 자동 품질 & 스케일아웃 레이어

AE-ADK v2.6.0+는 AE가 **자율적으로** 호출하는 두 가지 Claude Code 네이티브 스킬을 통합합니다 — 플래그나 수동 명령이 필요 없습니다.
...
병렬 에이전트로 변경된 코드를 재사용 기회, 품질 문제, 효율성, CLAUDE.md 준수 여부 측면에서 검토하고 자동으로 수정합니다. 구성 없이 매 구현 주기 후 AE가 직접 호출합니다.

**`/batch` — 병렬 스케일아웃**

대규모 병렬 작업을 위해 격리된 git worktree에서 수십 개의 에이전트를 실행합니다. 각 에이전트는 테스트를 실행하고 결과를 보고하며, AE가 이를 합칩니다. 워크플로우별 자동 트리거 조건:
...
AE는 **전략적 오케스트레이터**입니다. 직접 코드를 작성하지 않고, 27개 전문 에이전트에게 작업을 위임합니다.

```mermaid
graph LR
    U["👤 사용자 요청"] --> M["🗿 AE Orchestrator"]
...
| **Builder** | 3 | agent, skill, plugin | 새로운 AE 컴포넌트 생성 |
...
AE-ADK는 Claude Code 구독 요금제에 맞춰 27개 에이전트에 최적의 AI 모델을 할당합니다. 요금제의 사용량 제한 내에서 품질을 극대화합니다.
...
```bash
# 프로젝트 초기화 시
ae init my-project          # 대화형 마법사에서 모델 정책 선택

# 기존 프로젝트 재설정
ae update                   # 각 설정 단계별 대화형 프롬프트
```

`ae update` 실행 시 다음 항목을 묻습니다:
...
AE-ADK는 Claude Code가 지원하는 **Sub-Agent**와 **Agent Teams** 두 가지 실행 모드를 모두 제공합니다.

```mermaid
graph TD
    A["🗿 AE Orchestrator"] --> B{"실행 모드 선택"}
...
AE-ADK는 프로젝트 복잡도를 자동으로 분석하여 최적의 실행 모드를 선택합니다:
...
```bash
/ae plan "대규모 기능"          # 자동: researcher + analyst + architect 병렬
/ae run SPEC-XXX                # 자동: backend-dev + frontend-dev + tester 병렬
/ae run SPEC-XXX --team         # Agent Teams 모드 강제
```
...
```bash
/ae run SPEC-AUTH-001 --solo    # Sub-Agent 모드 강제
```
...
Plan 단계에서 Run 단계로 전환 시, AE가 현재 실행 환경(cc/glm/cg)을 자동으로 감지하고, 구현 시작 전에 사용자가 모드를 확인하거나 변경할 수 있는 선택 UI를 표시합니다.
...
모든 서브커맨드는 Claude Code 내에서 `/ae <서브커맨드>`로 실행합니다.
...
| 서브커맨드 | 별칭 | 목적 | 주요 플래그 |
|-----------|------|------|------------|
| `project` | `init` | 프로젝트 문서 생성 (product.md, structure.md, tech.md, .ae/project/codemaps/) | — |
| `mx` | — | 코드베이스 스캔 및 @MX 코드 레벨 주석 추가 | `--all`, `--dry`, `--priority P1-P4`, `--force`, `--team` |
| `codemaps` | `update-codemaps` | `.ae/project/codemaps/`에 아키텍처 문서 생성 | `--force`, `--area AREA` |
...
| 환경 | 명령어 | 리더 | 워커 | 용도 |
|------|--------|------|------|------|
| Claude 전용 | `ae cc` | Claude | Claude | 최고 품질 |
| GLM 전용 | `ae glm` | GLM | GLM | 최대 비용 절감 |
| CG (Claude+GLM) | `ae cg` | Claude | GLM | 품질 + 비용 균형 |

> **v2.7.1 신규**: CG 모드가 **기본** 팀 모드로 변경되었습니다. `--team` 사용 시, `ae cc` 또는 `ae glm`으로 명시적으로 변경하지 않는 한 CG 모드로 실행됩니다.

> **참고**: `ae cg`는 tmux pane 레벨 환경 격리를 사용하여 Claude 리더와 GLM 워커를 분리합니다. `ae glm` 모드에서 전환 시, `ae cg`가 GLM 설정을 자동으로 리셋합니다 — 중간에 `ae cc`를 실행할 필요 없습니다.
...
```bash
/ae fix       # 단일 패스: 스캔 → 분류 → 수정 → 검증
/ae loop      # 반복 수정: 완료 마커 감지까지 반복 (최대 100회)
```
...
**버그 수정:**
```
/ae fix (또는 /ae loop) → /ae review → /ae sync
```
...
AE-ADK는 개발 세션 중 Task 도구 메트릭을 자동으로 캡처합니다:

- **위치**: `.ae/logs/task-metrics.jsonl`
...
| 명령어 | 설명 |
|--------|------|
| `ae init` | 대화형 프로젝트 설정 (언어/프레임워크/방법론 자동 감지) |
| `ae doctor` | 시스템 상태 진단 및 환경 검증 |
| `ae status` | Git 브랜치, 품질 메트릭 등 프로젝트 상태 요약 |
| `ae update` | 최신 버전으로 업데이트 (자동 롤백 지원) |
| `ae update --check` | 설치 없이 업데이트 확인 |
| `ae update --project` | 프로젝트 템플릿만 동기화 |
| `ae worktree new <name>` | 새 Git worktree 생성 (병렬 브랜치 개발) |
| `ae worktree list` | 활성 worktree 목록 |
| `ae worktree switch <name>` | worktree 전환 |
| `ae worktree sync` | 업스트림과 동기화 |
| `ae worktree remove <name>` | worktree 제거 |
| `ae worktree clean` | 오래된 worktree 정리 |
| `ae worktree go <name>` | 현재 셸에서 worktree 디렉토리로 이동 |
| `ae hook <event>` | Claude Code 훅 디스패처 |
| `ae glm` | GLM 5 API로 Claude Code 시작 (비용 효율적 대안) |
| `ae cc` | GLM 설정 없이 Claude Code 시작 (Claude 전용 모드) |
| `ae cg` | CG 모드 실행 — Claude 리더 + GLM 팀원 (Claude Code 자동 시작, tmux 필수) |
| `ae version` | 버전, 커밋 해시, 빌드 날짜 정보 |

---

## Claude x GLM 멀티 LLM

AE-ADK는 **z.ai GLM**을 Claude Code의 대안 AI 백엔드로 지원하며, 멀티 LLM 개발 워크플로우를 구현합니다.
...
| **자동 실행** | `ae cg`가 현재 pane에서 Claude Code를 자동으로 실행합니다. `claude`를 별도로 실행할 필요 없음. |
...
|--------|--------|---------|-----------|-----------|---------------|
| `ae cc` | Claude | Claude | 아니오 | - | 복잡한 작업, 최고 품질 |
| `ae glm` | GLM | GLM | 권장 | ~70% | 비용 최적화 |
| `ae cg` | Claude | GLM | **필수** | **~60%** | 품질 + 비용 균형 |
...
AE-ADK는 **@MX 코드 레벨 주석 시스템**을 사용하여 AI 에이전트 간 컨텍스트, 불변 계약, 위험 영역을 전달합니다.
...
AE statusline은 버전 정보와 업데이트 알림을 함께 표시합니다:
...
**업데이트 방법**: `ae update` 실행 시 업데이트 알림이 사라집니다.

**참고**: Claude Code의 빌트인 버전 표시(`🔅 v2.1.38`)와는 다릅니다. AE 표시는 AE-ADK 버전을 추적하며, Claude Code는 자체 버전을 별도로 표시합니다.
...
`.ae/config/sections/statusline.yaml`을 직접 편집하세요:
...
> **참고**: v2.7.8부터 `ae init`/`ae update` 마법사에서 세그먼트 프리셋 선택 UI가 제거되었습니다. 위의 YAML 파일에서 직접 설정하세요.
...
**이유:**
- 프로젝트의 `.ae/config/sections/`에 이미 이 파일들이 존재합니다
...
- [Issues](https://github.com/modu-ai/ae-adk/issues) — 버그 리포트, 기능 요청
