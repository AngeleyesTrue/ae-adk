...
function Download-Binary {
    ...
    $tmpName = "ae-install-$([System.Guid]::NewGuid().ToString())"
    ...
    # Find the binary (ae on Unix, ae.exe on Windows)
    $isWin = ($IsWindows -or ($null -eq $IsWindows -and $env:OS -eq "Windows_NT"))
    if ($isWin) { $binaryName = "ae.exe" } else { $binaryName = "ae" }
...
function Install-Binary {
    ...
    if ($env:MOAI_INSTALL_DIR) {
        ...
        $targetDir = Join-Path $env:LOCALAPPDATA "Programs\ae"
    } else {
...
function Verify-Installation {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "    ae init          # Initialize a new project" -ForegroundColor Cyan
            Write-Host "    ae doctor        # Check system health" -ForegroundColor Cyan
            Write-Host "    ae update --project # Update project templates" -ForegroundColor Cyan
        }
    }
    catch {
