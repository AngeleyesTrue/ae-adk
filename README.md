<p align="center">
  <img src="./assets/images/ae-adk-og.png" alt="AE-ADK" width="100%">
</p>
...
<p align="center">
  <a href="https://github.com/modu-ai/ae-adk/actions/workflows/ci.yml"><img src="https://github.com/modu-ai/ae-adk/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
  <a href="https://github.com/modu-ai/ae-adk/actions/workflows/codeql.yml"><img src="https://github.com/modu-ai/ae-adk/actions/workflows/codeql.yml/badge.svg" alt="CodeQL"></a>
  <a href="https://codecov.io/gh/modu-ai/ae-adk"><img src="https://codecov.io/gh/modu-ai/ae-adk/branch/main/graph/badge.svg" alt="Codecov"></a>
  <br>
  <a href="https://go.dev/"><img src="https://img.shields.io/badge/Go-1.26+-00ADD8?style=flat&logo=go&logoColor=white" alt="Go"></a>
  <a href="https://github.com/modu-ai/ae-adk/releases"><img src="https://img.shields.io/github/v/release/modu-ai/ae-adk?sort=semver" alt="Release"></a>
...
AE-ADK is a **high-performance AI development environment** for Claude Code. 27 specialized AI agents and 52 skills collaborate to produce quality code. It automatically applies TDD (default) for new projects and feature development, or DDD for existing projects with minimal test coverage, and supports dual execution modes with Sub-Agent and Agent Teams.
...
We completely rewrote the Python-based AE-ADK (~73,000 lines) in Go.
...
AE-ADK implements the **Harness Engineering** paradigm — designing the environment for AI agents rather than writing code directly.

| Component | Description | Command |
|-----------|-------------|---------|
| **Self-Verify Loop** | Agents write code → test → fail → fix → pass cycle autonomously | `/ae loop` |
| **Context Map** | Codebase architecture maps and documentation always available to agents | `/ae codemaps` |
| **Session Persistence** | `progress.md` tracks completed phases across sessions; interrupted runs resume automatically | `/ae run SPEC-XXX` |
| **Failing Checklist** | All acceptance criteria registered as pending tasks at run start; marked complete as implemented | `/ae run SPEC-XXX` |
...
| **Garbage Collection** | Periodic scan and removal of dead code, AI Slop, and unused imports | `/ae clean` |
| **Scaffolding First** | Empty file stubs created before implementation to prevent entropy | `/ae run SPEC-XXX` |
...
```bash
git clone https://github.com/modu-ai/ae-adk.git
cd ae-adk && make build
```

> Prebuilt binaries are available on the [Releases](https://github.com/modu-ai/ae-adk/releases) page.
...
/ae project                            # Generate project docs (product.md, structure.md, tech.md)
/ae plan "Add user authentication"     # Create a SPEC document
/ae run SPEC-AUTH-001                   # DDD/TDD implementation
/ae sync SPEC-AUTH-001                  # Sync docs & create PR
/ae github issues                      # Fix GitHub issues with Agent Teams
/ae github pr 123                       # Review PR with multi-perspective analysis
```

```mermaid
graph LR
    A["🔍 /ae project"] --> B["📋 /ae plan"]
    B -->|"SPEC Document"| C["🔨 /ae run"]
    C -->|"Implementation Complete"| D["📄 /ae sync"]
    D -->|"PR Created"| E["✅ Done"]
```

---

## AE Development Methodology

AE-ADK automatically selects the optimal development methodology based on your project's state.
...
> The methodology is automatically selected during `ae init` (`--mode <ddd|tdd>`, default: tdd) and can be changed via `development_mode` in `.ae/config/sections/quality.yaml`.
>
> **Note**: AE-ADK v2.5.0+ uses binary methodology selection (TDD or DDD only). The hybrid mode has been removed for clarity and consistency.

### Auto Quality & Scale-Out Layer

AE-ADK v2.6.0+ integrates two Claude Code native skills that AE invokes **autonomously** — no flags or manual commands required.
...
Uses parallel agents to review changed code for reuse opportunities, quality issues, efficiency, and CLAUDE.md compliance, then auto-fixes findings. AE calls this directly after every implementation cycle — no configuration needed.

**`/batch` — Parallel Scale-Out**

Spawns dozens of agents in isolated git worktrees for large-scale parallel work. Each agent runs tests and reports results; AE merges them. Auto-triggered per workflow:
...
AE is a **strategic orchestrator**. Rather than writing code directly, it delegates tasks to 27 specialized agents.

```mermaid
graph LR
    U["👤 User Request"] --> M["🗿 AE Orchestrator"]
...
| **Builder** | 3 | agent, skill, plugin | Creating new AE components |
...
AE-ADK assigns optimal AI models to each of 27 agents based on your Claude Code subscription plan. This maximizes quality within your plan's rate limits.
...
ae init my-project          # Interactive wizard includes model policy selection

# Reconfigure existing project
ae update                   # Interactive prompts for each configuration step
```

During `ae update`, you'll be asked:
...
AE-ADK provides both **Sub-Agent** and **Agent Teams** execution modes supported by Claude Code.

```mermaid
graph TD
    A["🗿 AE Orchestrator"] --> B{"Select Execution Mode"}
...
AE-ADK automatically analyzes project complexity and selects the optimal execution mode:
...
```bash
/ae plan "large feature"          # Auto: researcher + analyst + architect in parallel
/ae run SPEC-XXX                  # Auto: backend-dev + frontend-dev + tester in parallel
/ae run SPEC-XXX --team           # Force Agent Teams mode
```
...
```bash
/ae run SPEC-AUTH-001 --solo      # Force Sub-Agent mode
```
...
AE's core workflow consists of three phases:
...
When transitioning from Plan to Run phase, AE automatically detects the current execution environment (cc/glm/cg) and presents a selection UI for the user to confirm or change the mode before implementation begins.
...
All subcommands are invoked within Claude Code as `/ae <subcommand>`.
...
| Subcommand | Aliases | Purpose | Key Flags |
|------------|---------|---------|-----------|
| `project` | `init` | Generate project docs (product.md, structure.md, tech.md, .ae/project/codemaps/) | — |
| `mx` | — | Scan codebase and add @MX code-level annotations | `--all`, `--dry`, `--priority P1-P4`, `--force`, `--team` |
| `codemaps` | `update-codemaps` | Generate architecture docs in `.ae/project/codemaps/` | `--force`, `--area AREA` |
...
|-------------|---------|--------|---------|----------|
| Claude-only | `ae cc` | Claude | Claude | Maximum quality |
| GLM-only | `ae glm` | GLM | GLM | Maximum cost savings |
| CG (Claude+GLM) | `ae cg` | Claude | GLM | Quality + cost balance |

> **New in v2.7.1**: CG mode is now the **default** team mode. When using `--team`, the system runs in CG mode unless explicitly changed with `ae cc` or `ae glm`.

> **Note**: `ae cg` uses tmux pane-level env isolation to separate Claude leader from GLM workers. If switching from `ae glm`, `ae cg` automatically resets GLM settings first — no need to run `ae cc` in between.
...
```bash
/ae fix       # Single pass: scan → classify → fix → verify
/ae loop      # Iterative fix: repeats until completion marker detected (max 100 iterations)
```
...
```
/ae plan → /ae run SPEC-XXX → /ae review → /ae coverage → /ae sync SPEC-XXX
```

**Bug Fix:**
```
/ae fix (or /ae loop) → /ae review → /ae sync
```

**Refactoring:**
```
/ae plan → /ae clean → /ae run SPEC-XXX → /ae review → /ae coverage → /ae codemaps
```
...
AE-ADK automatically captures Task tool metrics during development sessions:

- **Location**: `.ae/logs/task-metrics.jsonl`
...
| Command | Description |
|---------|-------------|
| `ae init` | Interactive project setup (auto-detects language/framework/methodology) |
| `ae doctor` | System health diagnosis and environment verification |
| `ae status` | Project status summary including Git branch, quality metrics, etc. |
| `ae update` | Update to the latest version (with automatic rollback support) |
| `ae update --check` | Check for updates without installing |
| `ae update --project` | Sync project templates only |
| `ae worktree new <name>` | Create a new Git worktree (parallel branch development) |
| `ae worktree list` | List active worktrees |
| `ae worktree switch <name>` | Switch to a worktree |
| `ae worktree sync` | Sync with upstream |
| `ae worktree remove <name>` | Remove a worktree |
| `ae worktree clean` | Clean up stale worktrees |
| `ae worktree go <name>` | Navigate to worktree directory in current shell |
| `ae hook <event>` | Claude Code hook dispatcher |
| `ae glm` | Start Claude Code with GLM 5 API (cost-effective alternative) |
| `ae cc` | Start Claude Code without GLM settings (Claude-only mode) |
| `ae cg` | Launch CG mode — Claude leader + GLM teammates (auto-starts Claude Code, tmux required) |
| `ae version` | Display version, commit hash, and build date |

---

## Claude x GLM Multi-LLM

AE-ADK supports **z.ai GLM** as an alternative AI backend for Claude Code, enabling multi-LLM development workflows.
...
│  - Orchestrates workflow when /ae --team runs             │
...
| **Auto Launch** | `ae cg` automatically launches Claude Code in the current pane. No need to run `claude` separately. |
...
|---------|--------|---------|---------------|--------------|----------|
| `ae cc` | Claude | Claude | No | - | Complex work, maximum quality |
| `ae glm` | GLM | GLM | Recommended | ~70% | Cost optimization |
| `ae cg` | Claude | GLM | **Required** | **~60%** | Quality + cost balance |
...
AE-ADK uses **@MX code-level annotation system** to communicate context, invariants, and danger zones between AI agents.
...
### Configuration (`.ae/config/sections/mx.yaml`)
...
```
🤖 Opus 4.6 │ 🔅 v2.1.74 │ 🗿 v2.7.12 │ ⏳ 5h 32m │ 💬 AE
CW: 🔋 █████████████████████░░░░░░░░░░░░░░░░░░░ 52%
5H: 🔋 █░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░ 4%
7D: 🔋 ██████████████████████░░░░░░░░░░░░░░░░░░░ 56%
📁 ae-adk-go │ 🔀 main │ 📊 +0 M38 ?2
```

**Default mode** (3 lines — 10-block inline bars):
```
🤖 Opus 4.6 │ 🔅 v2.1.74 │ 🗿 v2.7.12 │ ⏳ 16m │ 💬 AE
CW: 🔋 ██░░░░░░░░ 25% │ 5H: 🔋 █░░░░░░░░░ 12% │ 7D: 🔋 ░░░░░░░░░░ 3%
📁 ae-adk-go │ 🔀 fix/my-feature │ 📊 +0 M38 ?2
```
...
Edit `.ae/config/sections/statusline.yaml` directly:
...
> **Note**: As of v2.7.8, segment preset selection has been removed from the `ae init`/`ae update` wizard. Configure segments directly in the YAML file above.

---

### Q: What does the version indicator in statusline mean?

The AE statusline shows version information with update notifications:
...
**To update**: Run `ae update` and the update notification will disappear.

**Note**: This is different from Claude Code's built-in version indicator (`🔅 v2.1.38`). The AE indicator tracks AE-ADK versions, while Claude Code shows its own version separately.
...
```
External imports:
  /Users/<user>/.ae/config/sections/quality.yaml
  /Users/<user>/.ae/config/sections/user.yaml
  /Users/<user>/.ae/config/sections/language.yaml
```

**Recommended action**: Select **"No, disable external imports"** ✅

**Why?**
- Your project's `.ae/config/sections/` already contains these files
...
- [Issues](https://github.com/modu-ai/ae-adk/issues) -- Bug reports, feature requests

---

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=modu-ai/ae-adk&type=date&legend=top-left)](https://www.star-history.com/#modu-ai/ae-adk&type=date&legend=top-left)
